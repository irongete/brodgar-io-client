...
| **`Sheet`** | one addon's stylesheet — the whole of `hafen.ui.skin{…}` (033): parse a Lua table of selector→properties **once**, classify each key site-vs-tree (`Fonts.isScope`), and fill the `haven.Fonts` owner-tagged stack. Properties are `font` + `color` (033.2), each independently optional. Keys go through `Selector`, so the grammar and its errors are `hafen.ui(sel)`'s. The **tree** half resolves per widget into a `WeakHashMap` cache (034.1, read by `widget:style()`) and reaches the draw through `Fonts.treeStyles` → `specOf`, which the provider asks for every widget it draws (034.2) |
