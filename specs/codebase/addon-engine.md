...
| File | Owns |
|---|---|
| `AddonManager` (hub, ~1.6k lines) | lifecycle (attach/init/tick), the `haven` seams (`onUimsg`/`onWdgmsg`/`onMessage`/`onGlobKey`/`onWidgetPlaced`/`onWidgetRemoved`/`onWidgetResized`/`onBeltSet`/`onGhostClick` — core edits call these by name; they delegate; `onWidgetCreated` went with the descriptor in 032.2), the shared gob-read/engine substrate (`getgob`/`gobMatches`/`gobSnapshot`/`gui`/…), event bus + `callLua`, timers, the `:lua` REPL, `installHafen` |
...
| `UiApi` | `hafen.ui` — a **callable** namespace (`hafen.ui(sel)`/`.all(sel)`/`hafen.ui()`, `root()` cut in 030.1): windows/overlays, selector events, the window-toggle seam + the `widget:replace(view)` substitution and its per-tick sweep, the entry points + selector resolution + hit-testing, the container-subscription poll (`adopt` deleted in 029.2; `hafen.ui.replace` + `LuaReplacer`/`LuaModel` in 032.2). **Resolution since 049.2**: `collect` is the only walk — `firstMatch` is gone, because `selectFirst` must see the whole tree to know a match is the only one — and `one()` is the shared nil/one/**raise** split every `find` door returns through (D-224), its message quoting the caller's own door (`hafen.ui():all(…)` vs `widget:all(…)`). `collect`/`selArg` are package-private so `LuaWidget`'s `scopedFind`/`scopedAll` reach them with the same grammar and errors |
| **`Selector`** | the selector grammar (030.1, made real CSS by 049.1): parse **once** into a `Step[]`, then match one widget. A step is `*`/role + `@Class` + `[title=]`/`[text=]`/`[res=]`, each refiner an `Attr(op, val)` over the four operators `=`/`*=`/`^=`/`$=`; the steps are separated by whitespace **outside brackets** (the descendant combinator — captions contain spaces, so the split tracks bracket depth). Matching is greedy right-to-left up `parent` and needs no backtracking (D-221); `matchesStructure`/`late()` answer for the whole chain. `[title=]` is a `window`-only key and `[text=]` skips a `Window` (D-222/D-223), so `windowTitle`'s upward walk is **gone**. `specificity()` sums the chain and `bare()`/`role()` are what `Sheet.siteOf` rides — no consumer reads its fields. Pure — no Lua, no state — so the events, the sheet and the inspector reuse it. The widget→role classifier itself lives in `LuaWidget.role` (D-067), beside `typeName`/`text`/`resName` |
| **`Sheet`** | one addon's stylesheet — the whole of `hafen.ui.skin{…}` (033): parse a Lua table of selector→properties **once**, classify each key site-vs-tree (`Fonts.isScope`), and fill the `haven.Fonts` owner-tagged stack. Properties are `font`/`color` (033.2), `bg`/`border` (035.1) and `pad` (035.2) — parsed by `Chrome`, drawn by `SkinDeco`/`SkinBox` — plus `pos`/`size` (036.2), each independently optional. Keys go through `Selector`, so the grammar and its errors are `hafen.ui():find(sel)`'s — and since 049.1 a key may be a **chain**, which `siteOf` treats as a tree key by construction (a font scope is one bare word, so `bare()` decides it) with `Rule.rank` taking `sel.specificity()` summed over the steps. The **tree** half resolves per widget into a `WeakHashMap` cache (034.1, read by `widget:style()`) and reaches the draw through `Fonts.treeStyles` → `specOf`, which the provider asks for every widget it draws (034.2) — on the **drawing** half alone, so a layout-only sheet never opens a frame |
...
| **`Section`** + `LuaPagina` / `LuaItem` / `LuaGobOverlay` / `FlowerMenuApi` | `Section.mount`/`.install` are the two doors every `hafen.<name>` comes through — the **only** place to enumerate the live sections (14 + 17); the rest are the entity classes behind `hafen.menugrid()` (`pag:use` is gated at [`LuaPagina.java:301`](src/io/brodgar/addon/LuaPagina.java:301)), an inventory Item, one attached gob overlay (the **spec table is gone** since 039.3 — records are built bare and configured by setters, [`LuaGobOverlay.java:84`](src/io/brodgar/addon/LuaGobOverlay.java:84)) and `hafen.flowermenu()` |
...
| `UI.AddWidget` (`onWidgetPlaced(id, wdg)`) | the widget-**placement** seam: `hafen.ui.on` selector events (030.2), the layout cascade (`Layout.placed`, 036.2) and, since 042, every `TreeAdapter` that used to poll for a structural change — each with its own fast-path flag and, where needed, a bounded re-check for a late `[title=]`. The `UI.NewWidget` edit and the `{id,type,place,caption,parentType}` descriptor went with `hafen.ui.replace` (032.2) |
| [`GameUI.addchild`](src/haven/GameUI.java:910) | HUD placement switch (place-string routing) |
| `Widget.remove()` (`onWidgetRemoved(this)`, 042.1, D-179) | the widget-**removal** seam, override-proof unlike `cdestroy` (9 of 17 overrides skip `super`) — drains one frame's worth onto every M1 consumer (`TreeAdapter.removed`, selector `disappear`, `Layout`'s pruning, `WidgetSubs`/replace-sweep). Reused from a second call site in `Buff.reqdestroy`/`Window.reqdestroy` for a widget that FADES (D-180), whose unlink lags the server's real "gone" moment |
| `Widget.resize(Coord)` (`onWidgetResized(this)`, 042.10, D-181, supersedes D-091) | the **geometry** seam, past `Widget`'s own no-op check — an anchor target resizing, a window packing, or the screen changing. Reused from a second call site in `Window.resize` (`Window` skips `super.resize` for its own chrome sizing) |
| `GameUI`'s deferred `setbelt`/`setbelt2` lambdas (`onBeltSet(slot)`, 042.6, D-178) | the one case where the uimsg tap fires *before* the write lands (`glob.loader.defer`) — the notify goes inside the lambda, after the assignment: "the seam goes where the write LANDS" |
| `Loading implements Waitable` ([`Waitable.waitfor`](src/haven/Waitable.java:32)), wrapped by `Resolve` (042.1, D-182) | the **resolution** seam for a value still streaming: retry-on-notify (never wait-once), marshalled onto the tick (P5), every `Waiting` owned by its `Addon` for `:reload`/disable cancellation (P2). A bare `new Loading(…)` throws `UnwaitableEvent` — stated as a boundary (D-092), never chased |
