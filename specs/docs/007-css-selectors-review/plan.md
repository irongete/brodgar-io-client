...
4. **The standard, then the close.** Settle the refused spellings against D-013, re-derive the
...
**Created**
- `specs/docs/007-css-selectors-review/census.md` — 007.1's table, both directions.
...
- The ten `049` wrote: `api/ui/selectors.md`, `api/ui/widget.md`, `api/ui/replace.md`,
  ...
  `guides/debugging.md`, `guides/theming.md`, `examples.md` — corrections only (007.1).
- The fifteen it did not: `api/ui/items.md`, `api/ui/native.md`, `api/ui/custom.md`,
  ...
  `guides/events-and-timers.md` — plus `runtime.md`, which quotes a lookup in a `:lua` line (007.2).
- Whatever §12 turns up tree-wide (007.3).
...
- `decisions/docs-standard.md` — the D-013 outcome as D-019, whichever way it goes.
...
- **The stale prose is rarely on the page that changed (003.1, 006.3).** `038` left `player.md`
  ...
  007.2 greps the **prose names** of this grammar — "selector", "the first match", "the enclosing
...
- **The refused spellings are the false-zero's twin (006.1).** `inventory[title=…]` reads 1 on a
  ...
  reads zero *and* catches a reintroduction, or D-013's second half says it does not — and 004.2's
...
- **Re-running `005`/`006`'s corrections.** Out of scope: their surfaces were checked against `src/`
  at their close, and §12 whole-tree at 007.3 is what re-covers them cheaply.
