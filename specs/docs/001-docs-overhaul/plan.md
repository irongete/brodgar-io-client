# 001-docs-overhaul — Plan

## Approach

**Evidence, then standard, then one pass per section group.** The audit comes first and edits
nothing: a coverage matrix built from `specs/addons/FEATURES.md` + `STATE.md` (features 001–036,
...
- `specs/docs/001-docs-overhaul/audit.md` — the coverage matrix (feature × where documented) and
  the 39-page inventory. Written by 001.1, re-run at the close.
- `specs/docs/design/style-guide.md` — page template, voice, examples, ceiling, link rules.
- `specs/docs/design/information-architecture.md` — the target tree + the migration map.
...
- `specs/docs/{STATE,FEATURES,LEARNINGS}.md` — updated by `/end`; learnings go to
  `specs/docs/learnings/docs-maintenance.md` (new category file).
...
- A 36-feature matrix is the biggest single unit here. If 001.1 cannot finish it in a session,
