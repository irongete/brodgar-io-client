...
**The headline finding, and the spine of 005.4.** `src/io/brodgar/addon/Retired.java` is a
...
decision, D-015, and it retires the maintenance problem rather than the list.
...
**Scope control**: `005.1` produces a worklist and the two accuracy tasks work only from it. A
page not on it is touched by `005.4`'s tree-wide sweep alone.

## Files to create / modify

- `specs/docs/design/style-guide.md` — §7 rewritten as derived from `Retired.java`; §2's "no
...
- `specs/docs/decisions/docs-standard.md` — **D-015** (§7 is derived from the engine's refusal
  table), and D-016 if §2's scoping is judged a change of rule rather than a clarification
- `specs/docs/design/information-architecture.md` — `api/vr/**`, `api/event.md`,
...
- `docs/**` — the pages `005.1`'s worklist names; plus the six over-110 lines the ROADMAP names
- `specs/addons/_template/spec.md:20` — the `docs/addons/api/*.md` tier spelling (ROADMAP)
- `specs/docs/learnings/docs-maintenance.md`, `STATE.md`, `FEATURES.md`, `ROADMAP.md` — the close
- **Flag, not a change:** `CLAUDE.md:16` carries the same stale tier spelling but sits outside
  this area's commit paths (`docs specs`). `005.4` reports it; the maintainer says whether it
  rides the commit.
...
- **A §7 entry's proof has a shelf life** (`004.2`): `:offset(` was admitted against a zero tree
  ...
  directions at `005.4`, and the derivation is re-run, not trusted.
...
- **§2's word rule is the one place this review could manufacture work.** 152 `still` hits are
  ...
  is to scope the rule in `005.4` and fix only what is genuinely a change-note.
