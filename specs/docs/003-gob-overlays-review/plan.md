# 003-gob-overlays-review — Plan

## Approach

**Two tasks, accuracy first.** `003.1` corrects what the pages say about the code, reading `src/`; `003.2`
...
heading *is* a rename. D-006 scopes the annotation to write groups anyway, and `gob:overlay` is your own
...
today, which 003.2 re-verifies before the edit rather than after.
...
- `specs/docs/design/style-guide.md` — §7's grep list gains the proven spellings
- `specs/docs/STATE.md` `FEATURES.md` `ROADMAP.md` `LEARNINGS.md` (+ `learnings/docs-maintenance.md`) — `/end`
...
- `## Overlays (ungated)` — an annotation in a heading is a rename of a ten-link anchor, and D-006 does not
