...
- [ ] **No measured figure and no count in prose that duplicates a list** (§11 D-008, §6 D-012):
...
- [ ] **`gob:overlay` is classified as what it is.** It is not a row in `gob.md`'s `## Read` table while
      ...
      rather than a write (D-006 scopes the heading annotation to write groups; `## Overlays` keeps its
...
- `specs/docs/design/style-guide.md` — the standard enforced (§3 page kinds, §4 headings, §6 facts, §7 no history, §11, §12 checks)
- `specs/docs/design/information-architecture.md` (rules 2–4, namespace → path; §8 link discipline) ·
  `specs/docs/decisions/docs-standard.md` (D-008, D-009, D-010, D-012, opened per entry, never whole)
- `specs/docs/learnings/docs-maintenance.md` — the sweep traps: the double-hyphen slug, the over-reporting checker
- `specs/docs/002-map-database-review/` — the same job for `037`: the obituary-to-boundary rewrite and the grep-list extension
...
- `specs/addons/038-gob-overlays/spec.md` + `tasks.md` — what shipped, and what each claim is checkable against
