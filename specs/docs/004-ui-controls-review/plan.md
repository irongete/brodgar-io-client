# 004-ui-controls-review — Plan

## Approach

Two tasks, in the order `003` used, and for the same reason: **truth first, shape second**.

**004.1 — accuracy and the roster.** Read the two new pages and the four edited ones against `src/`,
...
**004.2 — shape, the standard's own files, and the close.** Settle the gating question as a decision
entry; decide the `controls.md` size question with 004.1's number in hand; teach the IA the pages
...
004.1's deletions buy enough headroom and the page stands; the page is two subjects (the spine plus
...
and the `g:` verbs plain because your own drawing changes no client state, and D-010 makes a
...
- `specs/docs/design/information-architecture.md` — §3's tree, §4's `api/ui` reading order
- `specs/docs/design/style-guide.md` — §7's grep list, extended with what `040` retired
- `specs/docs/decisions/docs-standard.md` — **D-014**, the gating of a control you own
- `specs/docs/STATE.md`, `FEATURES.md`, `LEARNINGS.md` (+ `learnings/docs-maintenance.md`) — the close
