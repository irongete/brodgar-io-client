...
- Neither new page uses the reference skeleton's `## Read` / `## Write (…)` groups — `controls.md`
  has `## Builders` and `## Setters`, carrying no gating annotation (§3, D-010).
...
- [ ] **`controls.md` is under the ceiling with headroom, split by subject if it splits** — and if it
      splits, every inbound link and anchor is re-pointed in the same task (§8, D-001, IA rule 3).
- [ ] **The IA and the style guide match the tree they describe**: §3's tree and §4's reading order
      ...
      (`entry:text()` is one) under D-013's admission rule.
...
- `specs/docs/design/style-guide.md` — the standard being enforced (§3, §6, §7, §9, §10, §12)
- `specs/docs/design/information-architecture.md` — §3's tree, §4's reading orders, rules 2-4
- `specs/docs/decisions/docs-standard.md` — D-001, D-006..D-010, D-012, D-013 (open only these)
- `specs/docs/learnings/docs-maintenance.md` — grep it; 001.4/002.1's re-point and re-wrap prices,
  001.7's two-click traversal, 003.1's terminator fix, 001.2's registration-string oracle
- `specs/docs/003-gob-overlays-review/` — the closest prior art: same shape, two tasks
- `specs/addons/040-ui-controls/` — `spec.md`, `tasks.md`, `api-sketch.md`: the roster to check against
