...
**Three core edits total**, each a one-line delegate into the hub, each tagged `// addon:`:
`Widget.remove` (042.1), the two `GameUI` deferred-belt lambdas (042.6), `Widget.resize` (042.10).
...
Verified in-game by the maintainer, per `specs/addons/TESTING.md`.
...
**Per task.**
- [ ] Each task ships its self-checking addon per `specs/addons/TESTING.md`; its run is all `[pass]`
...
- `specs/docs/design/style-guide.md` §9-§12 and `grep "^### D-" specs/docs/decisions/docs-standard.md`
