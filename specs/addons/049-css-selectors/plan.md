...
- `specs/addons/design/22-ui-selectors.md` — a superseding banner: its "no descendant selectors, no
  operators" scope line and its `[title=]` ancestor rule are both reversed here
- `specs/addons/decisions/widgets-ui.md` — the new decisions (see tasks.md)
