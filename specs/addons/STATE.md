...
**Active:** `021-actionbar-oop` — **021.1 DONE** (LuaSlot + hard cut); next 021.2 (`ActionbarChanged` fires a Slot) then 021.3 (docs + `hello`). Prior: `020-kin-oop` **DONE** (020.1–020.3: hard cut, Kin ↔ Gob, `KinChanged` = `Kin[]`); ROADMAP's OOP migration continues. (`019-profiling` **DONE**, 019.1–019.8.)
...
- **Gobs (OOP, D-044/045)**: `hafen.gob(id)` → an interned Gob
  ...
  `.slot/.use` GONE. **Map** `map.*`: tile/height/grid/gridPos + conversions, grid ids = exact decimal strings.
