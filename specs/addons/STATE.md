...
**ACTIVE: [`043-vr-namespace`](043-vr-namespace/)** — **4 of 5 tasks** (043.1–043.4 done), and its sibling
...
making the **anchor an argument**. **043.1 landed the section itself** — `:ghost()/:sprite()/:object()`, each
answering every verb its old section answered, the kinds **registered** rather than branched on (044's
...
supersedes D-034's half. **043.2 made the anchor an argument** — `:add(what, p)` stands, `:add(what, gob)`
follows, anything else refused naming **both**; an anchored entity dies with its gob off the `GobRemoved` drain
through a by-target index (**D-185**, an index written at create/destroy, not the sweep D-100 deleted), and
`:position(p)` on a follower is **refused** (**D-186**). **043.3 emptied `gob:overlay()` of the world** —
`ov:image`/`ov:model`/`ov:ghost` and the verb set serving only them are `Retired` rows naming `hafen.vr()`, so
`ov:offset(x, y)` means screen pixels and a third argument raises; the one world verb with no home moved with
them as `<entity>:offset(x, y, z)` (**D-187** — grep the fields the deleted door SOLELY wrote, not just the call
sites: `followOff` was one, and `tagger`'s pin would have dropped to the ground). An anchored entity is
**listed at its gob read-only**, every write refused naming its collection (**D-188**, a door is a write, a list
is a read), which deleted `asOverlay` whole. So `gob:overlay()` means exactly **what is drawn at this gob**;
`tagger` and `:hello follow` ported.
**043.4 gave the section the two verbs no single collection can be asked** — `hafen.vr():list(filter)` is
everything you have stood, across the three kinds, in **creation order** (the `eid` serial each entity carries)
and taking the same canonical filter a per-kind list takes; `hafen.vr():visible(b)` switches the whole section
off and back, destroying nothing. The restore is **D-189**: a section-wide switch is a **second boolean beside
the thing's own**, never a write over it, ANDed where the scene slot is added — so one hidden on its own handle
stays hidden, a `:visible(b)` written while the section is off is remembered, and one placed while it is off is
created and listed and waits. `<entity>:visible()` and `hafen.vr():visible()` are different questions and
neither is "is it on screen". Both verbs walk one registry sweep, so 044's fourth kind joins for free.
**The docs tier still describes `hafen.ghost`/`hafen.render`, batched into 043.5** — which owes the anchor
argument and its refusals, the death-with-gob rule, `<entity>:offset(x, y, z)`, the two verbs above and a
rewritten `api/gob.md` Overlays section, none of it in `docs/` yet. 043 still owes `:billboard(b)` →
`:facing(mode)` and that sweep (043.5). **044 adds the fourth**,
`hafen.vr():widget()`: a Widget — yours or the client's own — standing in the world as a quad, held to one rule,
**transparency** (same `Draw`, input, controls, theme, popups, keyboard), which is why its surface is a real UI
**root** the widget is reparented into rather than a texture with clicks forwarded; plus `:facing("camera")`, a
world quad turning in yaw and pitch with real size, perspective and occlusion. Every rendering piece exists.
