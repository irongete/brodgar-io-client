...
**Active:** `023-menugrid-oop` — the action menu (`MenuGrid` paginae) as OOP Actions. **023.1 DONE** (read surface, engine only — no docs/harness yet): `hafen.menugrid()` = the catalogue (interned, grid sort order, `:find/:roots/:list`), `hafen.menugrid(key)` = one Pagina — key by SHAPE (`/` ⇒ res name = identity, else display name; a number errors, a miss ⇒ `nil`); `:res/:name/:path/:parent/:children/:tooltip/:hotkey/:isnew/:exists/:info`. The catalogue is `paginae` **plus the parent closure** (categories live only in the private `pmap`); `Loading`-guarded ⇒ short right after login, fills in sub-second. **Next:** 023.2 `:use()`, then 023.3 docs + `hello`. Prior: `022-actionbar-set` **DONE** (022.1: gated `slot:set(resourceName)`, async write, `walker` demo). Prior: `021-actionbar-oop` **DONE** (021.1–021.3: LuaSlot + hard cut, `ActionbarChanged` = a Slot, docs + `hello` contract check), `020-kin-oop` **DONE** (020.1–020.3: hard cut, Kin ↔ Gob, `KinChanged` = `Kin[]`), `019-profiling` **DONE** (019.1–019.8). **Next:** ROADMAP's OOP migration continues (party/fight/… still flat).
