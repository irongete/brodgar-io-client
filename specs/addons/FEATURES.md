...
- 023-menugrid-oop — ACTIVE — the action menu (MenuGrid paginae) as OOP: `hafen.menugrid()` = every known action, `hafen.menugrid(resName|displayName)` = one interned Action (string key split by shape — a "/" means resource; no addressing by position, the catalogue grows on discovery), both keys → the same object (`:res/:name/:path/:parent/:children/:tooltip/:hotkey/:isnew/:exists/:info` + `:use()` driving `PagButton.use`, which reaches the id-only paginae no path can express; no `mods` — that path ignores `Interaction.modflags`), collection `:find/:roots/:list` — design/07's deferred menu-ability primitive, and the first section to own its own write verb ahead of the `hafen.act.*` restructuring — tasks 023.1 (read surface, DONE) .. 023.3
