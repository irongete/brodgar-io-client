...
- 019-profiling — ACTIVE — hafen.client:profiling(): frame/CPU/GPU timings, memory, render, net, loader, per-addon cost + custom Lua scopes, armed by a single master switch (zero cost when off); new Options "Client" panel with "Enable profiling", also at options():client(); per-widget cost, named render passes (shadow/scene/ui2d, CPU+GPU) + armed GL counters; profiler example addon (design/10) — tasks 019.1..019.8
