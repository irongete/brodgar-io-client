...
Verified in-game by the maintainer, one login per task, through `specs/testing/addon-suite.md`: each task ships
...
- [ ] Each task ships its self-checking addon per `specs/testing/addon-suite.md`; its run is all `[pass]` (plus
