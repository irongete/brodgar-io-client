...
- an anchor switch leaves the previous session's `AddonRoot` in its tree — `init` attaches a new one and only drops the field — and `Sessions.tick` ticks every session that is not drawn, so the whole engine step (Update, timers, every drain) runs once per switch per frame, for the life of the client (filed: 073)
- `VrApi.groundDirty` is one flag for the client while the free-entity list it gates is one per session, so the first session to drain it consumes the re-ground for every other — dormant only because the ground question itself (`groundDrawn` → `screenView()`) still answers for the drawn scene alone, so both have to take a session in the same change (filed: 073)
- `AddonManager.tick`'s EnterWorld gate reads the DRAWN HUD (`gui()`) while the `enterWorldPending` flag it is gating is one session's, so a session that reaches the world while another is on screen fires its `EnterWorld` against a HUD that is not its own — and the character folder its saved data lands in (`StoreApi.restorePerChar`, handed that same HUD) is then another character's. Dormant only because `init` binds the pump to the anchor alone (filed: 073)
- `Layout.capDirty` is one flag for the client while the pending list it gates is one per session, so the first session to drain it clears it for every other — the late `[title=]`/`[res=]` layout re-check of a second pumping session never runs, and there is no symptom but a window that quietly does not lay out (filed: 073)
