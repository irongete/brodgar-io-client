...
- [x] **054.1 — Accuracy, and the roster both ways.** Read `controls.md` and `lists.md` line by line
      ...
      **Reports**: every correction with its `src/` citation · the 16-builder roster, both directions ·
      the prose counts and duplicates removed · `wc -l` on every page touched, which is 054.2's input ·
...
- [x] **054.2 — Shape, the standard's own files, and the close.** Four decisions and a sweep.
      **(a) Gating** — settle whether a setter group on a control you built carries `(ungated)` or stays
      plain, and write it into `decisions/docs-standard.md` as **D-244**; §3's `hafen.ui.overlay`
      precedent, D-240 and 001.3's "an annotation everywhere is a signal nowhere" are the three inputs.
      Apply it to both pages' group headings.
      **(b) Size** — with 054.1's count, either `controls.md` stands with headroom, or it splits by
      ...
      **(c) The standard's files** — `information-architecture.md` §3's tree and §4's `api/ui` reading
      ...
      (`entry:text()`) under D-243's rule, falsified as reading zero on a healthy tree.
      ...
      **Reports**: D-244 and the reasoning that fixed it · the size decision with the number behind it ·
      ...
      <!-- extra context: `specs/052-map-database-review/` — 002.1 split `map.md` into a
