...
**056.1 — the census.** One row per rehoused verb (`048/spec.md` is the map) plus `047`'s three reads,
...
**056.2 — the standard.** The tier shipped the new adjective and `specs/` did not. `style-guide.md`
...
retired spelling; the IA's §3 tree loses `act.md` and gains `flowermenu.md`. **D-240 and D-244 are not
rewritten** — a new decision records the adjective and that both read in it, on `005`'s D-245/D-246
precedent.

**056.3 — the ceiling.** Three pages at 300/300/299 and `048` wrote into all three. Split **by subject**
(D-231), the candidate seams being `conventions.md`'s `## References: how you address things` (~96 lines,
...
**056.4 — the sweep and the close.** §12's seven checks over the whole tree, each re-derived: the retired
...
- `docs/addons/api/flowermenu.md`, `player.md`, `gob.md`, `menugrid.md`, `world.md`, `conventions.md`,
  `ui/items.md`, `ui/widget.md`, `event.md` — the `047`/`048` surfaces, corrected where 056.1 says so
...
- new pages from 056.3's splits + every page linking into a moved heading
- `specs/standards/docs.md` — §3, §4, §6, §7 (and §12 if the checklist wording follows)
- `specs/standards/docs-ia.md` — §3's tree, the `api/` reading order, the two
  `ungated` lines
- `specs/decisions/docs-standard.md` — one new decision (the adjective)
- `specs/056-act-dissolved-review/census.md` — 056.1's table, the feature's evidence
- `specs/STATE.md`, `FEATURES.md`, `learnings/docs-maintenance.md` — the close
...
- Rewriting D-240/D-244 in the new adjective — a decision records what was decided; a later one
  supersedes it (D-245/D-246 precedent).
- Splitting the ceiling pages by line count, or moving overflow into an appendix — D-231 is subject.
