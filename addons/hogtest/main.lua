...
-- Once armed it burns ~15 ms of Lua on EVERY Update -- over the per-tick budget (default 10 ms) but
...
local hafen = hafen
local BURN = 0.015   -- seconds of Lua to burn per Update when armed
...
hafen.event():on("Load", function()
  local cfg = hafen.store():get("cfg")           -- account-scope vars are loaded before Load (Phase 1e)
  ...
  if cfg.arm then
    hafen.log():write("hogtest ARMED -- burning ~" .. math.floor(BURN * 1000)
              .. "ms/Update; expect an auto-disable + AddOns-panel warning shortly")
    hafen.event():on("Update", hog)
  else
