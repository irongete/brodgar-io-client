...
--     agreeing to the millimetre.
--   * marker:anchor() IS WHY. A marker's own position is a segment id this client invented with
--     rnd.nextLong() plus a tile coord inside it, and a segment merge rewrites BOTH in place — so it cannot
--     be saved or sent: it would not go nil, it would point at the wrong place. :anchor() converts it to the
--     {gridId, x, y} that can be. This suite stores exactly that, and nothing in segment space.
...
local function sweep(seg, sc)
  local seen, n = {}, 0
  for _, g in ipairs(seg:grids{ x = sc.x - SWEEP, y = sc.y - SWEEP, w = (SWEEP * 2) + 1, h = (SWEEP * 2) + 1 }) do
    seen[g:id()] = true
    n = n + 1
  end
  return seen, n
...
local function run(args)
  ...
  check((type(sid) == "string") and (sid:match("^%-?%d+$") ~= nil) and (hafen.map.segment(sid) == seg),
        "the player's segment answers an exact decimal-string id, and map.segment(id) is the SAME object",
        sid)
  ...
  -- 6. marker:anchor(): the position that may be saved or sent
  local a = m and m:anchor()
  check((a ~= nil) and (a.gridId == (gp and gp.gridId))
          and near(a.x, (math.floor(gp.x / TILE) * TILE) + (TILE / 2), 0.001),
        "marker:anchor() names the SERVER grid id and the tile centre inside it",
  ...
  refuses("a 64-bit id is a decimal STRING, and a number is refused rather than rounded",
          function() return hafen.map.segment(1122) end, "decimal STRING")
  ...
  hafen.timer():after(1.5, function()
    ...
    check((sc ~= nil) and (seg:grid(sc) == g),
          "seg:grid(sc) hands back that same Grid object, so the two doors are one entity",
          sc and ("%d,%d"):format(sc.x, sc.y))

    -- 14. put the DB back exactly as it was found
    local removed = m and hafen.map.markers.remove(m)
    local still = false
    for _, x in ipairs(seg:markers(NAME)) do still = true end
    check((removed == true) and (not still) and (m:exists() == false),
          "the pin is out of the map DB again, and the marker says so itself",
          ("removed=%s still=%s exists=%s"):format(tostring(removed), tostring(still), tostring(m:exists())))
