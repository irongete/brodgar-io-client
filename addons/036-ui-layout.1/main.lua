...
--
-- Since 029 the Widget table has said that :pos(x,y) on a NATIVE widget is an error, "moving a native widget
  ...
  for _, w in ipairs(hafen.ui.all("*")) do
    ...
    if w:visible() and p and s and s.x > 32 and s.y > 32 and tried < 40 then
      ...
      if (hafen.ui.at(cx, cy) == w) and (hafen.ui.at(ux, uy) == w) then
        tried = tried + 1
        local was = w:pos()
        w:pos(was.x + dx, was.y + dy)
        if (w:pos().x == was.x + dx) and (w:pos().y == was.y + dy) then
          return w, was, ux, uy
        end
        w:pos(nil)
      end
...
local function run()
  ...
  local was = wnd:pos()
  local moved = pcall(function() wnd:pos(was.x + 37, was.y + 23) end)
  check(moved, "widget:pos(x,y) no longer refuses on a NATIVE widget -- feature E, the row 029 promised", moved)
  eq("...and it moved c for real, read back through the same verb",
     xy(wnd:pos()), ("%d,%d"):format(was.x + 37, was.y + 23))

  -- 3. the undo is the third arity, and it restores the stock numbers exactly.
  wnd:pos(nil)
  eq("widget:pos(nil) puts the window back exactly where the user had it", xy(wnd:pos()), xy(was))
  ...
  if t ~= nil then
    ...
    t:pos(nil)
    eq("...and it goes back where it was", xy(t:pos()), xy(tp))
  end
  ...
  eq("...and a layout write on it CHAINS rather than throwing", ghost:pos(9, 9) == ghost, true)
  eq("...and reads back nothing at all", ghost:pos(), nil)
  ...
  refuses("widget:pos(x) is a mistake, not an undo: the error names all three arities",
          function() wnd:pos(5) end, "widget:pos(nil)")
  refuses("widget:destroy() still refuses on a native widget", function() wnd:destroy() end, "NATIVE widget")
  refuses("...and the refusal now points at widget:pos(x, y), not at a later feature",
          function() wnd:pack() end, "widget:pos(x, y)")

  -- 8. nothing of ours is left on anything we did not move.
  wnd:pos(nil)
  eq("widget:pos(nil) on a window this addon is not holding is a silent no-op", xy(wnd:pos()), xy(was))
