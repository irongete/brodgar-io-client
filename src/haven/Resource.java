...
public class Resource implements Serializable {
    ...
    /* addon: the Brodgar resource store, a bucket behind a CDN with no logic of its own. Every
     * version of a resource is an object named after it: <name>.res.v<ver> (immutable) and
     * <name>.res (the latest the store knows). A 404 is the store saying "not yet", and the pool
     * moves on to the next source, the proxy, which fetches it and fills the store. */
    public static class StoreSource extends HttpSource {
	public StoreSource(URI base) {
	    super(base);
	}
    ...
    /* addon: a -U at the Brodgar store makes the network sources [store, proxy]: the store answers
     * straight from the CDN, the proxy (STORE_FALLBACK) fills what the store has not got yet. The
     * local cache keeps the proxy's identity (see HashDirCache.create), so switching -U from the
     * proxy to the store re-downloads nothing. Any other -U is a plain HttpSource, as before. */
    public static final String STORE_HOST = "res.brodgar.io";
    public static final URI STORE_FALLBACK = Utils.uri("http://brodgar.io/res/");

    public static boolean isstore(URI uri) {
	return((uri != null) && (uri.getHost() != null) && uri.getHost().equalsIgnoreCase(STORE_HOST));
    }
