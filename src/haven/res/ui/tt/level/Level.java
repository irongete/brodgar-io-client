...
/* >tt: Level */
/* addon: (064) UNMODIFIED local copy of the resource's own source (`haven.Resource get-code ui/tt/level`), adopted
 * so `contents:level()` can read the fill meter by TYPE. The two counts live in `cur`/`max` and nothing in `haven`
 * reaches them: the class ships inside the resource, and the engine only ever asks it for `overlay()`, the bare
 * fraction it paints over the icon. The pin is what makes the read safe -- a revised resource stops matching
 * (`ResClassLoader.loadClass`), so the verb answers nil rather than something wrong. */
