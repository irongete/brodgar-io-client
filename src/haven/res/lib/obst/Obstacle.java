 ...
 * the @FromResource line get-code writes. It is here so that the addon bridge can read the obstacle a
 * construction site carries in its SDT -- see gob:hitbox() -- by running the resource's OWN parser
 * rather than by decoding those bytes itself, which docs/addons/api/gob.md says this API does not do.
 *
 ...
 * the original behaviour rather than breaking. Nothing here is modified; the reader that calls it is
 * io.brodgar.addon.AddonManager.sdtRings. */
