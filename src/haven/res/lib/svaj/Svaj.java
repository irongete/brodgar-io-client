...
/* rts: a LOCAL COPY of the resource's own code (doc/resource-code), verbatim and unmodified. It is here
 * only because GobSvaj beside it had to be modified and cannot compile without this type; the engine
 * prefers a local copy only when the @FromResource name and version match the resource actually served,
 * and otherwise warns and uses the fetched code, so a server-side bump degrades to the original
 * behaviour rather than breaking. The bug this pair exists for is in GobSvaj -- see there. */
