...
public class MCache implements MapSource {
     ...
     * how the maintainer sees whether a dormant session is streaming at all. Aligning two sessions'
     * frames is a different question and is asked elsewhere: Sessions.Member.prove walks the grids
     * themselves, because a Grid carries both `gc` (this session's, login-relative) and `id` (the
     * server's) and only the id means the same thing in both. */
    public int numgrids() {
	synchronized(grids) {
	    return(grids.size());
	}
