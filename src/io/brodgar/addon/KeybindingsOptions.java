 ...
 * {@code register("test", ...)} then {@code get("test")} refer to the same binding without the addon ever
 * spelling its own id. A name that has no addon-scoped binding falls back to the raw registry id, which is how
 * a built-in client hotkey is reached ({@code get("inv")}, {@code set("inv", "Ctrl+I")}). The addon scope is
...
public final class KeybindingsOptions {
    ...
    public static LuaValue create(final Addon owner) {
        ...
        mt.set(LuaValue.INDEX, methods(owner, kb));
        kb.setmetatable(mt);
        return kb;
    }
    ...
    private static LuaTable methods(final Addon owner, final LuaValue handle) {
        ...
        // get(name) — the binding's current key as a display string ("Ctrl+M"), or nil when unbound/unknown.
        m.set("get", new VarArgFunction() {
            public Varargs invoke(Varargs a) {
                LuaValue name = a.arg(2);
                if(!name.isstring())
                    throw new LuaError("keybindings:get(name) expects a string");
        ...
        // set(name, key) — remap a binding, exactly as the keybind panel does (persisted; "None" unbinds).
        m.set("set", new VarArgFunction() {
            public Varargs invoke(Varargs a) {
                LuaValue name = a.arg(2), key = a.arg(3);
                if(!name.isstring() || !key.isstring())
                    throw new LuaError("keybindings:set(name, key) expects (string, string)");
                KeyBinding b = resolve(owner, name.tojstring());
                if(b == null)
                    throw new LuaError("keybindings:set: no binding named '" + name.tojstring() + "'");
                KeyMatch km = HookApi.parseKeyMatch(key.tojstring());
                if(km == null)
                    throw new LuaError("keybindings:set: cannot parse key '" + key.tojstring()
