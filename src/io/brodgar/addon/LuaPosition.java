 ...
 * {@code hafen.world():position(x, y)} produce; the anchor form is what comes back out of {@code hafen.store}
 * and {@code hafen.world():position(info)}. Neither form is primary and both answer the same verbs.
 *
...
public final class LuaPosition {
     ...
     */
    private static Anchor anchorAt(double x, double y) {
    ...
    /** The world coordinate of a grid's upper-left corner in THIS session, or {@code null}. */
    private static Coord2d ulOf(long id) {
        Coord2d ul = AddonWidgets.gridWorldUL(AddonManager.mcache(), id);
        if(ul != null)
            return ul;
    ...
    Anchor anchor() {
        return located ? anchorAt(wx, wy) : new Anchor(gridId, gx, gy);
    }

    // ---- construction ------------------------------------------------------------------------------

    /** A Position at a session world coordinate — what every live reader hands back. */
    static LuaValue ofWorld(Addon owner, double x, double y) {
    ...
    /** A Position at {@code rc}, or {@code NIL} when the caller had no coordinate. */
    static LuaValue of(Addon owner, Coord2d rc) {
        return (rc == null) ? LuaValue.NIL : ofWorld(owner, rc.x, rc.y);
    }
    ...
    static LuaPosition posArg(Varargs a, int i, String verb, String param) {
        ...
        if(p == null) {
            throw new LuaError(verb + ": " + param + " must be a Position — gob:position(),"
                + " hafen.world():position(x, y), or hafen.world():position(saved). A plain {x, y} table is"
    ...
    private static LuaTable methods(final Addon owner) {
        ...
        m.set("offset", new VarArgFunction() {
            public Varargs invoke(Varargs a) {
                ...
                Coord2d rc = p.world();
                return (rc == null) ? LuaValue.NIL : ofWorld(owner, rc.x + dx, rc.y + dy);
            }
        });
