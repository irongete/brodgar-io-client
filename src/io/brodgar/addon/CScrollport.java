    ...
    CScrollport(Addon owner) {
        ...
        this.cont = add(new Scrollport.Scrollcont(sz.sub(bar.sz.x, 0)) {
            public void update() {
                ...
                // 139.4: a range that narrowed re-clamps the position, silently -- the rule :range(min, max)
                // keeps, since a child shrinking is not something the user did. Written straight, not through
                // bar.ch(0), which reports Changed.
                if(bar.val > bar.max) {
                    bar.val = bar.max;
                    sy = bar.val;
                }
