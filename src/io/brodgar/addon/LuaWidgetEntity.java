...
public final class LuaWidgetEntity extends LuaWorldEntity {
     ...
     * the <b>live</b> root of <b>its own</b> tree ({@code surface.ui}, the session the widget stood from — a
     * widget must never be re-homed into another login's tree, and after a relogin the tree it belongs to is
     * gone entirely); the recorded parent must still be in that tree (else the widget goes to the root rather
     * than into a dead frame); and the content must still be in the surface at all.
     */
    void destroyed() {
        WidgetSurface s = surface;
        UI u = (s == null) ? null : s.ui;
