...
| Add/attach child | `Widget.add`, `add0`, `attach` — `attach(UI)` is what sets the public `Widget.ui` field, recursing the whole subtree, and `add0` calls it only where the child's is still null. So **every widget carries the `UI` whose tree it is in**, and the one to lock while writing a widget is `w.ui` rather than any field naming the session on screen ([multi-session.md](multi-session.md)). It is null on a widget that has never been added to a tree, and `remove()` does **not** take it back. **`add(T)` is the one door**: `add(T, Coord)` and `adda` reach it and `add0` is private, so a container that must see every child entering overrides `add(T)` and works after `super.add` — the coordinate the caller passed is already written by then. ⚠️ **And it is written by reference**: `Coord` is mutable and `add(T, Coord)` stores the very object, so a child added at `Coord.z` has `c == Coord.z` and a later `child.c.y = …` moves the client's shared zero — every widget ever placed at it and every sum that starts from it. Place a child at a fresh `Coord`, computed before the add, and never assign into `c`'s fields on one placed at a shared constant |
...
| Re-order a sibling | `Widget.raise`, `lower` — `unlink()` + `link()`/`linkfirst()`, and **each takes `synchronized(ui)` itself**, unlike the rest of the tree writes, which lean on the caller's. So they are the two a consumer may call holding nothing. ⚠️ **`link()` appends at the END of its z** — the loop stops at the first sibling whose `z` is not greater — so killing a child and `add`ing its replacement puts it below every sibling; a container whose child order means something keeps `prev` from before the kill and re-links after it by hand |
...
| **Pre-hook a widget's own event handling** | `Widget.listen`/`deafen` (a `CopyOnWriteArrayList<EventHandler.Listener<?>>`) + `Widget.handle(Event)` — runs every listener **before** `ev.shandle(this)` (the widget's own `mousedown`/etc.); a listener returning `true` short-circuits both the default handling *and* child propagation. Fork: the first line of `handle` is the **disabled cut** (`// addon:`, `AddonWidgets.disabled`), ahead of the listeners — a `MouseEvent` is answered `true` (swallowed: no listener, no `shandle`, no child walk, nothing beneath) and a `MouseMoveEvent` or a `KbdEvent` `false` (skipped: the walk goes on past the widget); a `TickEvent`, a `TooltipQuery` and a `CursorQuery` are not asked. `Widget.keydown`'s Tab cycle carries the read half (`AddonWidgets.enabled`), or Tab would land on a widget that takes no key |
...
| **Sizing to content** | `Widget.pack()` is `resize(contentsz())`, and `contentsz()` is the max bottom-right (`c.add(sz)`) over the **visible** children — so a leaf packs to `(0, 0)`. `Window` overrides `contentsz()` (see [chrome](ui-chrome.md)). `resize` returns early on an equal box, then cascades `presize()` to the children and calls `parent.cresize(ch)` — **both are empty in `Widget`, and `Window` overrides neither**, so resizing a window's child triggers no relayout of the window. Read the other way, `cresize` is the seam a container that measures its children overrides: `Label.settext` resizes through `Widget.resize` and so reaches it, but **`Window.resize` dispatches to its own `resize2` and never calls `super.resize`**, so a window's own resize reaches no parent's `cresize` — a container relying on that seam cannot hold a window |
