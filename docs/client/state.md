...
| **What a dropped render add costs, and what takes it back** | `MapView.Gobs.addgob` runs on a Loader thread through `Loader.defer(Runnable, T)` — the `capex = false` overload — so whatever it throws is rethrown out of `Loader.Future.run` and **kills that thread**, with the gob still parked in `Gobs.adding`, never in `current`, and so out of the render tree while the `OCache` goes on holding it, unhidden: an object simply absent from the scene until the whole set is rebuilt. ⚠️ **`Loading` is a `RuntimeException`**, and it is the one that must go on escaping — the `Future` waits on it and re-runs the task, which is the only retry there is — so it is caught and rethrown *above* the arm that bounds the rest, which issues a `Warning` naming the gob's id and its `Drawable` resource. What takes the object back is `Gobs.tick()`, a 0.25 s reconcile called from `MapView.tick` inside `synchronized(glob.map)`, off the dormant path and skipped while `slot` is null. It does both halves: **add** what the `oc` holds and the view has in neither `current` nor `adding` — both, or a gob whose deferred `addgob` has not run yet takes a second slot and paints twice a frame — and **evict** what `current` holds and the `oc` no longer does, under the gob's own monitor like every other caller of `removed(Gob)`. `Gobs.want(Gob)` is the single predicate for *does THIS view hold it*, which `SessionGobs` extends with `skipgob` and `vis` so the merged view's culling and the base's rule cannot disagree |
| Game object | `Gob`: `id`, `rc`, `a`, `getattr`, `getc` |
| Gob attributes | `GAttrib`: `Drawable.getres()`, `Moving`, `GobHealth.hp`, `GobIcon`, `Speaking` |
| Gob attrib map | `Gob.attr` is **package-private** (reflection from outside `haven`); keyed by `attrclass` = the subclass directly under `GAttrib`, so `getattr(C)` only matches that exact key — a same-named class from another loader misses (see `learnings/engine-lifecycle.md`). It is a `ConcurrentHashMap`, so **a null value is refused**: the delete path is `setattr(ac, null)` reaching `attr.remove(ac)` alone, and iteration ORDER is unspecified — `Gob.eqpoint` answers the first `EquipTarget` it walks past, so a gob carrying two may answer the other one |
| **Who writes the attrib map, and who walks it** | **Every write is under the gob's own monitor**, and the map is the funnel's alone: `Gob.setattr(Class, GAttrib)` and `Gob.delattr` are `synchronized`, and `setattr(GAttrib)` passes through the first. The monitor is there for **atomicity, not for the map** — `setattr` is a read-modify-write across two structures (`attr.remove`, `RUtils.multirem` of the displaced attrib's slots, `RUtils.multiadd` of the new one, `attr.put`, `prev.dispose()`), and two writers interleaving in it leave a gob whose attribs and whose render slots disagree. The map is concurrent for the **readers**, which hold nothing: `Gob.added(RenderTree.Slot)` and `Gob.ctick` walk `attr.values()` off the Loader thread, `ModSprite.omods` off whichever thread rebuilt the sprite's mods, `SpeakerIcon.nameTex` off the UI thread inside the render pass, and `Gob.eqpoint`/`Gob.dispose` off either. `OCache.GobInfo.apply` already holds the monitor when it calls `setattr` — monitors are reentrant, so the delta path takes it once |
| **Which render slots an attrib is standing in** | `GAttrib.slots` is **package-private to `haven`**, filled by `GAttrib.added`/`removed` as the tree hands the attrib its own child slots. So taking one attrib out of a gob already in the scene, or putting it back, cannot be written outside `haven` at all — the helpers are `RUtils.multiadd(gob.slots, node)` / `RUtils.multirem(attr.slots)`, exactly what `Gob.setattr` uses to swap a `Drawable`, and `multiadd` rolls its own partial adds back before rethrowing. Copy the collection before `multirem`: it is the very list `removed` mutates |
| **Retrying slot surgery that throws** | `Gob.defer(Runnable)` queues onto `Gob.deferred`, run from a `Loader.Future` — so a task that throws `Loading` (adding a `Drawable` whose sprite is still resolving) is **parked and re-run** with no retry loop of your own ([boot-and-loop.md](boot-and-loop.md)). The queue is FIFO and the task is only dequeued once it returns, so a parked one holds that gob's later deferrals behind it; the body runs under `synchronized(gob)` and is **skipped entirely once the gob is `removed`** |
| Kin mark on a gob | `res/ui/obj/buddy/Buddy` (`@FromResource` v4): public `id` = the `BuddyWnd.Buddy` id ([kin-window.md](kin-window.md)). Server-stamped on **every** gob of that kin — body **and** hearth fire (`gfx/terobjs/pow`) |
| Map / terrain | `MCache`: `gettile`, `tilesetr`, `getcz`, `getgrid`; `tilesz`/`cmaps` (/). **The height at an arbitrary world point** is `getcz(Coord2d)` — bilinear across the four surrounding tile corners, each `getfz(Coord)` off the `Grid` — and `getzp(Coord2d)` is that same read wrapped as the `Coord3f` the projection wants (`MapView.clipxf`/`screenxf` take one). Both go through `getgridt`, so ⚠️ **either throws `MCache.LoadingMap` for ground this client has not been sent** — a point outside the streamed area has no height at all, and a per-frame caller has to catch it rather than assume zero |
...
| Character attributes | `Glob.getcattr`, `CAttr{base,comp}`. The sheet that draws them, and the public `CharWnd.exp`/`enc` beside them, is [character-sheet.md](character-sheet.md) |
...
| Time / astronomy | `Glob.globtime()` (the client's interpolated clock) and `Glob.ast` → `Astronomy` `dt`/`night`/`mp`/`yt`/`is`, plus the light fields — how the clock converges on the server's, and what `is` means, is [game-clock.md](game-clock.md) |
...
- **A `catch(RuntimeException)` around slot surgery swallows the retry.** `Loading` extends
  `RuntimeException`, and every `RenderTree` add that reaches a `Drawable` may throw one — so an arm meant
  to bound a failure catches the *parking* instead, and an object whose model was merely still streaming
  is dropped as if it had failed. Catch `Loading` first and rethrow it. `RenderTree.SlotRemoved` is an
  `IllegalStateException`, so it needs its own arm above too, or the specific handling it already has is
  shadowed.
- **The lock order is gob, then tree — and `Gob.added(RenderTree.Slot)` must stay unsynchronized.**
  `MapView.Gobs.addgob` opens with `synchronized(ob)` and only then calls `slot.add`; `OCache.add`,
  `remove`, `ladd` and `lrem` hold it across their callback loops; `Gob.deferred` runs each queued task
  under `synchronized(this)`. `RenderTree.TreeSlot.add` calls `Gob.added` **while holding the tree**, so a
  monitor there would be tree-then-gob, the inversion this client deadlocks in — it needs none, because
  every caller of it already holds the gob. Anything new that touches `GAttrib.slots` goes the same way
  round: take the gob, then let `RUtils` reach the tree from inside.
