...
| Performance (fork) | The twelve `perf-*` prefs, each written live-and-persisted in one statement. Read by the seams in `Tileset`, `TerrainTile`, `MapMesh`, `MCache`, `Glob`, `Gob` and the `lib/svaj` copy. |
