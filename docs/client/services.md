...
| Keybinding registry (remappable, persisted) | `KeyBinding.get` (id → binding, process-global), `set`, `key()`, `all()` (fork addition), `Bindable`; stored as `keybind/<id>`. The `key` field is **three-state** — `null` means "use `defkey`" (what `key()` collapses), `KeyMatch.nil` means explicitly unbound, anything else is the assignment. ⚠️ **`set` is exclusive** (fork addition): assigning a real key `clear()`s every OTHER binding whose `key()` normalises to the same code and modmatch, `sameKey`/`keycode` treating a `forchar` and a `forcode` match as one. `get` runs no such pass, so **two defaults may share a key and both fire** — `inv` and `rts-next-anchor` are both Tab. That pair cannot be repaired once touched: writing either takes the key off the other, and writing it back only takes it off again, so a caller that saves-and-restores a key must first check `all()` for another id reading it |
