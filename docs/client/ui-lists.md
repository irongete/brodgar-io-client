...
| What | Where |
|---|---|
| The inner container | `Scrollcont`, `public static` — reusable directly, and sized `sz.sub(bar.sz.x, 0)` by the port's constructor: the content is the port's width **less the bar's** (`Scrollbar.width`, from `sflarp`), and a child laid out to the port's own width is clipped at that edge — a label at the right of a row loses its last word there, not at the port's; its clip+scroll draw is `draw(GOut)`, offsetting each child by `-sy` via `xlate` and skipping one whose translated box misses the port entirely |
| The bar's range, auto-derived | `Scrollcont.update()` (the constructor's override) sets `bar.max = max(0, contentsz().y + 10 - sz.y)` — runs from `Scrollcont.add`, and from `Scrollcont.cresize` and `cdestroy` (tagged `// addon:`), so a child that grows after it entered moves the range; **not** from the port's own `resize()`, which is the box changing and not the content. It moves `max` and nothing else: a `val` above the new `max` is not clamped, so a refill that shrinks the content leaves `cont.sy` pointing below it and the port draws blank until the wheel moves — a refill writes `bar.val` and `cont.sy` back to `0` itself |
