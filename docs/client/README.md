...
| [search lists](ui-search-lists.md) | `SSearchBox`: `allitems()` vs `items()`, typing, `search`/`stopsearch` and the `change` a search makes, and its four subclasses — kin, polity members, map markers, map icons |
...
| [weather, tree sway, overlay plumes and plant sprites](world-effects.md) | `Glob.wmap`/`Weather`/`Weather.Factory` and the two places that compose and tick it, the weather numbers `wmap` does not keep and the light `MapView.amblight` builds, the `lib/svaj` adopted copy and how `Gob.Placed.Placement` recomposes a sway, `Gob.Overlay`'s lazy `init()`/`slots` lifecycle a plume rides, and the `lib/plants`/`lib/gplant` factories a crop's sprouts come from |
...
| [ground detail](ground-detail.md) | how a cut's decoration is seeded — `Tileset.SpriteFlavor.flavor`'s per-tile reseed and draw, `MCache.Grid.makeflavor`/`Flavobjs` — what its mesh is made of — `TerrainTile.Blend`'s weights and `en` layers, `faces`/`_faces`, `MapMesh.build`'s two passes and `dotrans` — and the cut lifecycle: `Deferred.get` vs `cur`, `invalidate` vs `rebuild`, and the identity swap in `MapRaster.Grid.tick` |
...
| [terrain height](terrain-height.md) | where a height comes from and who reads it: `MCache.Grid.z` and the record, `MCache.getfz` as the drawn read and `getrealfz` as the streamed one, `getcz`/`getzp`, the `ZSurface` a cut hands out and the water's bottom, the placers, the camera, the placers that cache on `chseq`, and the ridge model (`Ridges`: broken edges, the five shapes, `connect`, `TexCons`) and how, flat, the cliff keeps its shape on the plane |
| [the camera](camera.md) | reading the installed camera outside a render pass, the name→class registry and the two prefs, and the frustum's two traps, with the default camera's planes that follow its distance |
| [render targets](render-targets.md) | what a map-view frame draws into, a second fragment output (`FragData`, its `null` = `GL_NONE`, per-output blending), the `RenderedNormals`/`Outlines` pattern of a buffer the scene writes and a `ScreenQuad` reads back, the draw-order table, and the output-numbering gotcha that made the program cache miss |
...
| [resource loading](resource-loading.md) | how a name becomes a `Resource`: the two pools and their parent link, the source order, the asking and the parsing version rules, the soft cache, `Loading` and the two failures, and the `load`/`init` order layers are built in |
| [the on-disk store](rescache.md) | `ResCache` and its two implementations: who writes which key family (`res/`, `map/`, `data/mm-icons-2/`), `HashDirCache`'s slots, header, lock and temp-and-rename, the gotchas a hash table of files has, and the `haven.store` seam that puts the map and the resource cache in two SQLite files beside the jar |
| [published code](published-code.md) | the Java a `.res` carries: the `code` layer, where a resource is fetched from and in what order, `@FromResource` adoption, and the gotcha that every public member of `haven` is an ABI the server's code links against by name and descriptor |
