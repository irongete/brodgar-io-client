...
| [terrain height](terrain-height.md) | where a height comes from and who reads it: `MCache.Grid.z` and the record, `MCache.getfz` as the drawn read and `getrealfz` as the streamed one, `getcz`/`getzp`, the `ZSurface` a cut hands out and the water's bottom, the placers, the camera, and the placers that cache on `chseq` |
