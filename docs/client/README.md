...
| [ground detail](ground-detail.md) | how a cut's decoration is seeded — `Tileset.SpriteFlavor.flavor`'s per-tile reseed and draw, `MCache.Grid.makeflavor`/`Flavobjs` — and the cut lifecycle: `Deferred.get` vs `cur`, `invalidate` vs `rebuild`, and the identity swap in `MapRaster.Grid.tick` |
