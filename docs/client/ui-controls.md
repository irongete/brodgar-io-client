...
| `state`/`set`/`changed`/`click` are all **public fields** | so a caller may replace any of them — `SDropBox` replaces `state`/`set` on its own drop arrow, which is why that arrow never runs `changed` at all. ⚠️ **They shadow the enclosing scope inside an anonymous subclass**: a local or parameter named `set`, `changed`, `state` or `click` (a `Consumer<Boolean>` handed to a helper that builds the box, say) is hidden there by the inherited field of the same name and type, so `set.accept(v)` in an overridden `set(boolean)` compiles and runs the box's own default — it ticks and writes nothing. Name such a variable anything else |
