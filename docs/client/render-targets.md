...
| A state installed in every scene program | `RenderContext.basic(id, op)` → `PView.basic(id, op)`: an op on the root of the tree, so every slot under it re-derives. Keyed by any object; `RenderedNormals.Canon` uses its own class. ⚠️ It applies the states again only when the op it is handed is not `Utils.eq` to the one standing, and an op is read when applied: setting the **same object** whose fields have moved does nothing. Upstream's `basic(Camera.class, camera)` every tick did nothing either, and the picture followed the camera only because `MapView.lights()` handed a new light state every frame; fork: the camera is set as a new op of its `proj` and `view` whenever either moves |
