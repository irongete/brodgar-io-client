...
| It does not take the addon engine | `RemoteUI.init` guards the `AddonManager.init` call. That hub is single-session — its `ui` and `view` are the client's one live pair — so a session that is not drawn binding itself there points every addon at a session nobody is looking at |
