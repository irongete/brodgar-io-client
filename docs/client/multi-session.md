...
| It does not fight over window geometry | Several `GameUI`s load the same keys, and a place is written only when the user drops a window (`Window.mouseup`). Only the drawn session takes the pointer, so an untouched session never writes back the positions it loaded, and nothing has to be written when the screen changes hands ([window-positions.md](window-positions.md)) |
