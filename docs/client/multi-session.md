...
| Everything not drawn | `Sessions.tick` then `Sessions.gtick(out)`, called **after** that block closes, each session under its own monitor, and the one holding the screen skipped because the frame above has already ticked it in full. One call and one loop each: every game session the client holds is a member of that list. The `gtick` is not optional — see the `TickList` gotcha below |
...
- **`TickList` does not call `Gob.gtick`.** It dispatches `TickList.Ticking.autogtick`, which `Gob.Placed`
  does not implement, so a gob's animated pose reaches the GPU through `OCache.gtick` alone — `SkelSprite.gtick`
  (its `gtickparts`) and every overlay sprite's `gtick` have no other caller. `Composited.Model.autotick`
  does push a skeletal pose, but a plain `SkelSprite` drawable and every effect overlay do not go that way.
  `Glob.gtick` has exactly one caller for the drawn UI (`UILoop.Frame.tick`), so a member drawn into the
  anchor's scene needs `Sessions.gtick` every frame or its animated objects show whatever their buffers last
  held — frozen mid-swing until that session takes the screen, and correct the moment it does.
