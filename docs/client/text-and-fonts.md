...
| **The composition scope** | `Fonts.enter(scope)`/`Fonts.exit()` push a per-thread stack a routed site declares itself with; `Fonts.scope()` is the innermost one else `"default"` (what the generic `Text.render`/`RichText.render` statics resolve), and `Fonts.dynamic()` is the innermost one, else `"default"` while a **per-widget frame** is open and **null** otherwise, which is the form `Text.Foundry.resolved()` and its `RichText` twin ask — that is how a foundry the fork cannot route, a `.res`'s own private one, still resolves, and how a per-widget frame reaches it with no second mechanism. `enter` early-outs while `Fonts` is inactive AND the stack is empty, so the skip can only be taken outermost and an unmatched `exit()` finds an empty stack |
...
| **Wrapping** | `RichText.Foundry.render(text, width)`, in device pixels, and **`0` means do not wrap** — `Text.Foundry.render` lays a string on one line and nothing else, so a width forces the rich path (`Text.Foundry.renderwrap` reaches for the same one). `layout` splits any `Part` that overruns and `TextPart.split` keeps **at least one character** doing it (`// addon:`), because a head that is empty leaves an identical tail behind and `layout` never advances — an infinite loop that hangs the client on a width narrower than the first glyph, and on an `$img` wider than the box, where `Part.split` hands itself straight back. `layout` therefore splits each part **once** per line and lays an unsplittable one out overrunning, so a raster can come back wider than the width it was given. The two paths also measure a line differently — the stock one takes `FontMetrics.getHeight()`, the rich one the glyph bounds — so a wrapped line is a couple of pixels shorter than the same string unwrapped |
