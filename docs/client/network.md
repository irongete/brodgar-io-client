...
| Flower menu lifecycle (`@RName("sm")`) | `added` grabs mouse **and** keys (so nothing can be typed while one is up) and `organize(opts)` lays the ring out inside it — the END of `added` is the moment the petal set is complete and placed. `uimsg` `"act"`/`"cancel"` are the commit points; each starts an anim (`Chosen` 0.75 s / `Cancel` 0.25 s) that calls `ui.destroy` at `s == 1`, so the widget outlives its own close. ⚠️ **Both grabs come off at the commit, not at the destroy**, so for that quarter-to-three-quarter second the ring is in the tree holding no input at all and a second `sm` can be added beside it: `ui.root.children(FlowerMenu.class)` really can answer **two**, the dead one first. `BuddyWnd`'s `destroy()` **does** call `super` |
