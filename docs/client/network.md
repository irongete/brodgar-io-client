...
| **Which `UI` a send reaches: the widget's own** | `Widget.wdgmsg(sender, msg, args)` recurses up `parent` and only the **root** (`parent == null`) calls `ui.wdgmsg`, so the `UI` is the one that widget's tree hangs from — never an ambient "current" one. With several sessions live that is what makes a send driven off a widget land on **that** session: `BuddyWnd.Buddy.chname`/`chgrp`/`endkin`/`forget` and `Speedget.set` all reach the server through the login whose HUD holds them, drawn or not. `UI.wdgmsg` then runs the addon action-hook chain before `UI.rawWdgmsg`, which resolves the id against that same `UI`'s `rwidgets` |
