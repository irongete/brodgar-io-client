...
| **Ticking one yourself — `ctick`/`gtick` are called under the GOB’S OWN MONITOR** | `OCache.ctick`/`OCache.gtick` wrap every call in `synchronized(ob)`; that is the gob’s contract, not a detail of the cache. Both methods walk `Gob.ols`, a plain `ArrayList`, and `Gob.defer` runs the deferred half of `Gob.addol` and of `Gob.Overlay.remove` on a **loader thread** holding exactly that monitor (`Skeleton`’s `FxTrack` overlays are the ordinary source: an animation event adds one and the sprite’s end removes it, both async). ⚠️ **A gob outside `OCache` is ticked by whoever holds it, and the monitor is easy to drop on the draw half**: `MapView.tick` synchronizes on the `Plob`, and `MapView.draw` must too — unguarded, a frame drawn while an overlay is being added dies on `ConcurrentModificationException` inside `Gob.gtick`, killing the UI thread. Same for anything held beside `OCache` (`MapView.addClientGob`, the merged view’s own `gtick`) |
| **The parallel `gtick` (`Config.par`, default on) hands each pool thread a sub-`Render` through a `ThreadLocal` made per call** | `OCache.gtick`/`TickList.gtick`: the calling UI thread runs a leaf of the `parallelStream` itself, so it gets an entry in its own `ThreadLocalMap` per call, while the `commonPool` workers (`InnocuousForkJoinWorkerThread`) erase theirs after every task. ⚠️ **Upstream never removes it**: one dead entry per gtick, each holding a disposed sub-`Render`, and under ZGC's concurrent marking they are never cleared — an hour AFK grew a 2^18 table with one 49k-slot run beginning right after the slot of `MapFile.lock`'s read-hold `ThreadLocal`, so every contended read-unlock of the map file (`MiniMap.resolve` beside a `Defer` grid load) walked it in `expungeStaleEntry` for hundreds of ms, and the walk's own `Reference.get()` kept the keys alive; 364 of 527 live MB hung off it. Fork: both sites `remove()` it in a `finally` (`/* perf: */`) — a `/merge` that drops that line brings the stall back |
...
| **Where a gob IS — two answers, and only one of them is a field** | `Gob.rc` is the **server's** point, written at construction and by `Gob.move` (`OCache.$move`) and nowhere else. `Gob.getc()` is the **interpolated** one: `Moving.getc()` when a `Moving` attrib is on the gob, else `Gob.getrc()` = `placer().getc(rc, a)`, plus a `DrawOffset` that is **`z` only** (`DrawOffset.$zoff`). Every `Placer` — `DefaultPlace`, `InclinePlace`, `BasePlace`, `LinePlace` — answers with the `rc.x`/`rc.y` it was handed and computes only `z`, so the two answers differ in `x`/`y` **exactly while the gob interpolates**. ⚠️ **`rc` does not move between server messages**: `LinMove.getc` walks from `s` along `v` as `OD_LINSTEP` advances `t`, while `rc` sits where the last `OD_MOVE` put it — so anything that gates on `rc` alone freezes for a walking gob. The attrib is present exactly while that runs (`LinMove.$linstep` `delattr`s `Moving` when the step ends) and is the rest of the answer; the server's last point can arrive a tick BEFORE it goes, so the tick it goes on is its own edge. Worth the care: `getc()` descends `MCache.getzp` → `getz` → `getgridt`, building a `Coord3f` and three `Coord` per call, while `rc` is a field read |
| **Where a gob is placed and how it is TURNED — the per-drawable hook** | `Gob.Placer` (`getc(rc,a)` + `getr(rc,a)`→`Matrix4f`); `Gob.placer()` asks the `Drawable` first — `Drawable.placer()`, **public, overridable**, defaulting to `gob.glob.map.trnplace` (`MCache.trnplace`/`mapplace` = `Gob.DefaultPlace`, `getr` = z-rotation by `-a`). `Placed.autotick` rebuilds a `Placement` **every frame** and re-reads `getr`, pushing `slot.ostate` only when it differs (`Matrix4f.equals`) ⇒ **a per-frame rotation costs an override, not a tick loop**. Applied as `Location("gobx")` (translate, y-negated) then `Location("gob")` (this rotation), with `GhostGob.obstate`'s scale below both. Ignore facing entirely: `Location.nullrot`. **Per-gob render state on a NATIVE gob — the `SetupMod` seam:** `Gob.SetupMod` is an interface a `GAttrib` may also implement; `gobstate()`/`placestate()` contribute a `Pipe.Op` to the gob's own child slot, i.e. **below** the `"gobx"` translate and `"gob"` rotate of the row above ⇒ **T·R·S**, the same level `GhostGob.obstate` scales at. `Gob.setattr` registers it in the private `setupmods`; `Gob.ctick` calls the private `updstate()`, which rebuilds `GobState` from that list and pushes `slot.ostate` **only** when `Utils.eq(mods)` differs — so a change lands next tick and costs no new seam. ⚠️ **`Location` has no `equals`**: a freshly minted op each tick compares unequal every tick and re-pushes forever — **cache the op per value**. `Pipe.Op.compose` drops nulls and returns the lone survivor **itself** (only `Composed.equals` is `Arrays.equals`), so with one mod the comparison is identity. Copy `GobHealth`; an attrib that is **also** a `RenderTree.Node` is the only shape for which `setattr` can throw `Loading`. ⚠️ **A `State.Slot` holds ONE state, and `MixColor.slot` is `GobHealth`'s**: a second `MixColor` contributed by another `SetupMod` REPLACES the damage wash (whichever mod is applied last wins), it does not compose — a colour of your own is a `State` with a `Slot` of its own at another `FragColor.fragcol(...).mod` order, `ColorMask`'s shape |
...
| **A mask the client computes, rather than one a grid recorded** | `MCache.LocalOverlay` is a plain interface beside `RectOverlay` — `id()`, `fill(Area, boolean[])`, and the two defaults `filter(Area)` and `tick()`. `MCache.add`/`remove` register and unregister one. `MCache.getols` skips an overlay whose `filter(a)` answers **true** for the area being built, and the default answers `false`, i.e. *ask me about every cut*; `fill` then marks each tile of `a` the overlay covers, indexed `a.ri(lc)`. Fork: `filter` is also what decides which **cuts** are asked for — `MCache.olreaches(id, area)` puts it to one cut's tiles exactly, and allocates nothing of its own to do it — so a `filter` answering with `Area.isects` costs nothing, and one answering with `overlap(...) == null` builds two `Coord` and an `Area` per cut, per overlay, per frame to read one of them for its nullness. The **argument** is not free either: `MapView.Overlay.skipcut` mints it as `Area.sized(cc.mul(cutsz), cutsz).margin(1)` — two `Area` and three `Coord` — at that same rate. ⚠️ **A `filter` written generously now costs cuts, not merely a mask fill.** `olreaches` is one-sided: it may answer true where the mask misses, never false where it touches, so a grid that *records* the id answers **maybe** for all of it — a recorded mask is one boolean per tile of a 100×100 array, dearer to test than the `getolcut` it would save. Only a `ResOverlay` is ever recorded (`Grid.getol` matches `ols[i].get().layer(ResOverlay.class)`), so for every other `OverlayInfo` the locals decide alone and no grid is touched |
| ⚠️ **Who registers one — and when it can name itself** | `MCache.add`/`remove` arrive on **whatever thread the overlay’s owner runs on**, and resource-published gob code is the ordinary case: `gfx/fx/msrect`’s attrib constructor registers a `gfx/fx/mscover` from a **loader thread**, while `getols` and the cut build walk `MCache.ols` on the UI thread — so that set, and anything indexing it, is read and written concurrently. **`id()` answers `null` for the whole of that window**: the member reaches `add()` before its owner has finished constructing, and names itself a moment later. Two rules follow. **Read `id()` where you use it, never cache it at registration** — the key you would file it under is the wrong one, and a lookup by its real id then misses it, which for `olreaches` means answering *false* for a mask that reaches. **Never hand it straight to a `ConcurrentHashMap`** — `merge`/`get` throw `NullPointerException` on a null key, on the loader thread, killing the parse of the very attrib that was registering. A member that cannot name itself means what the single `MCache.olseq` has always meant: everything changed |
| **The once-per-tick walk of the whole map** | `MCache.ctick(dt)` — every loaded `Grid`'s `tick`, then every `LocalOverlay`'s. Called from `Glob.ctick`, i.e. the `stick` phase, so it runs **before** `ui.tick` reaches `MapView.oltick` in the same frame and long before anything is drawn ([boot-and-loop.md](boot-and-loop.md)). It is the one place per tick that reaches every `Grid.Cut` on the thread that builds them. The overlay half copies the whole of `ols` into a fresh `ArrayList` to walk it, so a client carrying many marks pays that copy every tick whether or not a single `LocalOverlay` overrides `tick()` |
...
| **World-anchored 2D blit (billboard)** | `Speaking` `extends GAttrib implements RenderTree.Node, PView.Render2D`; `draw(GOut,Pipe)` projects the anchor at `zo` up the gob's axis into a screen pixel (the `w > 0` test above, then `toview`) and blits it with `g.image(...)` — the speech bubble is the precedent for anything pinned above a head |
...
| **Lighting** | material state = `Light.PhongLight` (frag; ctor takes emi/amb/dif/spc/shine; `defamb/defdif/defspc` neutral defaults) → the `Phong` shader multiplies the scene's `Lighting.lights`/`Light.LightList` (applied at the PView scene root — `PView.lights`) into the fragment. The scene's light state is asked for every frame (`PView.draw` → `MapView.lights`), and upstream compiled it anew each time: for ZONED lighting `Lighting.LightGrid.compile` allocates a 64³ cell grid and a new `GridLights` with two new textures, disposing the last pair, and every program re-reads its light uniforms. Fork: the state standing is kept while the lights' `params()`, the projection and the `LightCompiler` hold — the compile is a function of those alone. Upstream's camera rode on that new state ([render-targets.md](render-targets.md), `basic`). Normals need the **inverse-transpose** of `basis·node` (`Matrix4f.invert`/`transpose`/`trim3`). **sRGB = no-op** (`Texture.srgb` left `false`, like all game textures). ⚠️ **A material with the `cel` spec (`Light.CelShade`, most of the game's objects and characters) does not follow the light's strength**: `Phong.CelShade.celramp` takes the brightest channel of the summed light and paints the face at full brightness above 0.5, at half from 0.1 to 0.5 and black below, keeping only the light's hue — so dimming the scene's light darkens the ground but leaves those objects as bright as by day until a step is crossed. Fork: `celramp` picks the step against the light divided by a scale uniform, and the step comes out dimmed by it (the `// addon:` line there) |
