...
| **The default camera** | Fork: `MapView.DefaultCam`, an orbit on `FreeCam`'s gestures, standing alone rather than extending it so that `RTSCam` keeps `FreeCam` as it is. Its options are the `MapView.dcam*` statics ([prefs-and-options.md](prefs-and-options.md)), read every tick: a proportional wheel, a **vertical** field of view (the horizontal derived from the aspect, where `Camera.resized` holds the horizontal), a projection rebuilt every tick for that reason — and, past the frustum's first trap below, one whose planes follow the eye's distance `d`: the far plane is `max(2000, 2.5 d + 1500)`, and past `d + MapView.recallreach()` while the view distance is on ([terrain-raster.md](terrain-raster.md)), the near plane `max(1, d / 200)` with the field scaled by it, so up close it is upstream's `1`..`2000` and first person keeps its one unit; `Projection.makefrustum`'s `2·far·near` is `d²/40` in float, so past `sqrt(Float.MAX_VALUE)` the view goes non-finite and an eased NaN never eases back, which is why the wheel refuses a target past it and a stored distance past it heals to the default —, collision, first person, a tilt below the horizon independent of the collision (off, the eye goes under the terrain), and the wheel's ease. The ground's limit is a hard ceiling on the eye's distance, since the ground only ever moves continuously; the objects', which cross the line in one frame, is eased in at the wheel's own pace (`dcamzsmooth`), and let out slower still, only after the way has stayed open a moment. **Collision** pulls the eye in along its own line to `CLEAR` short of whatever is first: the ground, sampled through `MCache.getcz` and bisected (ground the cache does not hold is not in the way, and is asked of `MCache.groundheld` first: `getcz` on an absent grid queues a map request before it throws), and the objects, whose `obst` rings bar `build` are stood up as prisms from `Gob.getc` to the top of the resource's meshes (`FastMesh.pbounds`). **First person** is `Camera.firstperson()`; `MapView.fpsync` withholds the player's `Drawable` through `Gob.camvisible`, a flag beside the addon layer's `addoninvis` on the same surgery, so neither undoes the other |
