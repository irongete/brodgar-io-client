...
| The contract, and the stock one | `Window.Deco` — `z(-100)`, `abstract iresize(Coord)` + `contarea()`; `DragDeco` adds the caption drag — its `mousedown` on the caption calls `Window.drag(ev.c)`, which takes `ui.grabmouse` into the window's private `dm`, and from then on `Window.mousemove` itself does `move(c + ev.c − doff)` until `mouseup` drops the grab: the window moves through nothing but its own `move`, and a subclass that has to see the drag compares `c` across `mousemove`, and sees its end only by overriding `mouseup` (`dm` is private, and `Window.mouseup` says nothing); `DefaultDeco` adds `drawbg`/`drawframe`, the close `IButton`, the sizer, `checkhit` and the margins (`dlmrgn`/`dsmrgn` + `tlm`/`brm`) |
...
| The corner sizer — the client's ONE user-driven resize | `DefaultDeco.dragsize` (off by default; `MapWnd` alone turns it on) + `szdrag`/`szdragc`. `mousedown` hit-tests a **triangle** at `ca.br` (`c.y >= ca.br.y - UI.scale(25) + (ca.br.x - c.x)`), takes `ui.grabmouse(this)` and stores `szdragc = aa.sz().sub(c)`; `mousemove` is then `((Window)parent).resize(ev.c.add(szdragc))` — the pointer plus a constant, so the window's own `c` is never touched and **a resize moves no origin**. The grab is the deco's, not the window's: the deco's own `mouseup` drops `szdrag` (private) and tells nobody, so a `Window` subclass sees neither the press nor the release, only its own `resize` calls — the one word it gets is `Window.resizedByHand(done)`, which the deco calls after each `resize` and once on release. `drawframe` paints `sizer` at `ca.br.sub(sizer.sz())` |
