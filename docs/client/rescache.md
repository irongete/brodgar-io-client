...
| The two files | Under `savedata/` beside the jar (`haven.savedatadir` moves it): `map.sqlite` for what `ResCache.global` holds — the `map/` keys and `data/mm-icons-2/…` — and `rescache.sqlite` for `res/`. Two lifecycles, two files: a cache wipe never runs inside the player's map. Each store keeps its own data — nothing is imported either way, and switching back shows the files store as it was left. Inside `map.sqlite`, one table per world: `GameUI.mapfilename()` puts the server's genus first in every key, `map/<genus>/…`, and such a row lives in the table `map-<genus>`, made when the world is first seen; everything else — `res/`, `data/`, a `map/` key with no genus — lives in `entries`. The tables are alike and the name stays whole, so a world's map is one table to dump, restore or drop with any SQLite tool. The layout is the file's `user_version` (2): a file at any other, older or newer, is refused with the reason and the client runs without that store for the session — nothing converts it |
| What the store promises `haven` | The same contract as the files: a miss is `FileNotFoundException` and only a miss; a `store` stream writes on `close()` and nothing otherwise, so an aborted download leaves no row where `HashDirCache` leaves a header-only slot; a zero-length blob round-trips as zero bytes. A file that cannot be opened — no driver or `java.sql`, an unwritable folder, a file at another schema — is one `Warning` naming it and the reason, and `null`: the degradation a `null` from `create()` already has. Two clients on one folder share a file through the write-ahead log and a busy timeout |
| `:store` | A static console command, registered by the store class's initialiser in both modes and printing through `cons.out` — the System channel in-world ([console.md](console.md)). `store: files` with the `data/` folder and the `HashDirCache` identities in use, or `store: sqlite` with a `map:` and a `res:` line — path, entry count and size, or `not open: <why>` — one `map-<genus>: <n> entries` line per world table under `map:`, and a `sweep:` line: `none` before a sweep finished (`running` while the first is on), else `examined <n>, dropped <m>` of the last one. `:store sweep` runs the sweep first, on the console's thread, and prints the same report; in files mode it says `sweep: files mode never sweeps`. Any other argument is refused naming both spellings. A client command, so `hafen.console():on("store")` is refused |
| The sweep | The pack (`brodgar-res.jar`) answers before the cache, so a `res/` row it holds at the same or a newer version is dead weight; the version is the little-endian `uint16` after the 16-byte `"Haven Resource 1"` signature. The store class's `sweep()`: a daemon thread runs it once from `resources()` in sqlite mode, right after the open, and `:store sweep` runs it now; runs serialise on one lock. It reads `SELECT id, name, substr(data, 17, 2) FROM entries WHERE name LIKE 'res/%'` — never the blob — copies the rows out under the instance monitor, then probes the pack per row without it, the lookup `JarSource.get` makes (`Resource.class.getResourceAsStream("/brodgar-res/<name>.res")`), reading the signature and the two bytes after it. Pack version ≥ row version → the row is deleted; the pack lacks the name, is not on the classpath, or its entry is not a resource, or the row is newer, or its blob is shorter than the version → kept. The deletes are one transaction, each `WHERE id = ? AND substr(data, 17, 2) = ?` with the bytes the sweep read, so a row the tee rewrote meanwhile — a version the pack lacked — stays. One stderr line per run, and the counts on `:store`. Measured: ~80 µs per probe over `bin/brodgar-res.jar`, so a cache the size of the pack sweeps in under a second. Files mode never sweeps: enumerating the folder is minutes |
