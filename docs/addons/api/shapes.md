...
| Rule | Detail |
|---|---|
| Every read is keyed | `kin:color()`, `segment:color()`, `marker:color()`, `overlay:color()`, `handle:color()`, `entity:tint()`, `gob:tint()`, `gob:outline()`, `rule:color()`: `.r` answers on all, `[1]` on none. A snapshot carries the same table under `color`, so a colour lifted out goes straight into a write. That is a [party member](types/world.md#partymember), a [kin](types/world.md#kinentry), a [meter](types/ui.md#meter) and its `segments`, a [marker](types/map.md#marker). |
| Both spellings go in | `rule:color(color)`, `marker:color(color)`, `overlay:color(color)`, `handle:color(color)`, an entity's `:tint(color)`, `gob:tint(color)`, `gob:outline(color)`, `graphics:text{color = color}`. `overlay:color(kin:color())` is one expression. Alpha defaults to `255`. A component outside `0..255` is clamped. |
