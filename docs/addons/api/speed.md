...
| `:list()` is empty in one situation | Before the client's speed selector streams in, a beat after entering the world: nothing at all, and `:get` answers `nil` too. The server locking every speed is different: `:list()` answers four, `:available()` is empty, `:set` refuses everything naming what is pickable. |
...
| The read-back is a round trip | The selected speed changes when the server says so: `:current()` answers the old speed for a beat after the call. Poll it. |
