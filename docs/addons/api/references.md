...
[`hafen.resource():get(name)`](resource/README.md) is a live handle for a well-formed resource name, minted
for any name and fetching nothing until a content read; `:list(filter)` walks the resources the client
holds. A [`Layer`](resource/layers.md), `resource:layers():get(key)`, is live while it is one of its
resource's layers and answers `false` on `:exists()` once a load or a [write](resource/writes.md) replaced
it.
