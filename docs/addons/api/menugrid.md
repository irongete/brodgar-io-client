...
A resource name never scans display names and a display name never matches a resource lookup. A miss is `nil` for both: that character does not have the action. A stashed handle still answers `:exists()` after the entry goes.
...
The catalogue is flat and complete: every action plus the categories they hang under. `:parent()` always lands on a readable entry. `:roots()` is never empty once the menu is up. "Is this a category" is `pagina:children():count() > 0`.
...
`pagina:on("Pressed", fn)` runs `fn(pagina)` every time one of your entries is pressed. That is a left-click on its button in the grid, its key on a held [action-bar slot](actionbar.md#hold-a-slot-unprotected), and [`pagina:use()`](#use-protected) from your own code.

```lua
local dig = hafen.session():current():menugrid():add("dig"):name("Auto-dig")
local subscription = dig:on("Pressed", function(pagina)
...
| Method | Returns | Permission | Description |
|---|---|---|---|
| `pagina:on("Pressed", fn)` | subscription | Unprotected | Run `fn(pagina)` when this entry is pressed. The same subscription [`hafen.event()`](event/README.md) hands you. |
| `subscription:off()` | the subscription | Unprotected | Unsubscribe. Idempotent, and done for you on reload or disable. |

| Rule | Detail |
|---|---|
| `Pressed` is the only key | The word a [button of yours](ui/controls/README.md#subscribing) fires on. Any other name throws at the line that wrote it. |
