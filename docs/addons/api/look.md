...
| Ends with the loaded object | Dropped when the object leaves its last character's view, the moment [`GobRemoved`](event/bus/world.md#world) fires: walk far enough for it to unload and it comes back as the game draws it, while another character still having it in view keeps it. Re-apply from [`GobAdded`](event/bus/world.md#world) to keep it across the unload. |
...
> **An object you hide is not there to be clicked.** A click resolves against the scene as drawn, and an object with no model draws nothing, so a click where it stood reaches the ground behind it. The verb never hides a model and keeps the click.
...
Everything drawn at a gob (the game's own, the labels and painters you attach, whatever you have [stood in the world](virtual/README.md) anchored to it) is the collection [`gob:overlay()`](overlay.md), unprotected. An overlay is attached to the object, so every character that can see the object draws it, and a `:reload` or a disable takes it off all of them.
