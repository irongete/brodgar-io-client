...
what it is made of, and what stands at it. The three verbs on this page change the first three and nothing
else — the object the server knows is untouched, and so is every read on the Gob page; the fourth override,
the [materials](materials.md) a model is drawn in, has a page of its own. All are **unprotected**: the change
is client-local and purely visual, on the same footing as an [overlay](overlay.md).
