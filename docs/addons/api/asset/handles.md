...
drawn, so it stands beside the client's own art at the same size at every interface scale. An image asset is
also what [`resource:layers():add`](../resource/writes.md) takes as an `image` or `tex` picture.
...
`.mid` or any other binary format you read as bytes. An `.ogg` data asset is what
[`resource:layers():add`](../resource/writes.md) takes as an `audio2` clip, and a `.res` one what
[`resource:layers(file)`](../resource/writes.md#whole-files) takes. It is how an addon's *content* stops being
written in Lua:
