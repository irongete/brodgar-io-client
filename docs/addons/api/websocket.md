...
A closed set of keys; a name outside it raises at `connection:on`, naming them. `Open` hands the connection; the others hand an `event` answering `event:connection()` (the object `:connection(url)` gave you, so `==` tells connections apart in a shared handler) and what the key carries.
