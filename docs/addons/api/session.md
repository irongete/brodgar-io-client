...
| [`s:store()`](store/vars.md) | its own vars: that character's rows in your addon's file |
...
the same thing. [`s:store()`](store/vars.md) is the vars of one character, that character's own rows
...
| `hafen.session():saved()` | `string[]` | the account names the login screen remembered ("Remember me"); empty when there are none, and never a token |
...
nothing to miss: a name read out of a [var](store/vars.md) hands back an object before that account
...
`hafen.session():add(user)` logs a remembered account in as a new session, without the login screen, using the
login the client saved when the player ticked "Remember me"; `hafen.session():forget(user)` is the login
screen's "Forget me" button. `:saved()` lists the accounts that have a saved login.
...
|---|---|---|---|
| `hafen.session():add(user)` | `Session` | `session.add` | logs the remembered account in, in the background, and hands back its `Session` at once — the same object `:get(user)` gives |
| `hafen.session():forget(user)` | the collection | `session.forget` | deletes the account's saved login and removes it from `:saved()`; chains |

* **The login runs in the background.** `:add()` returns before the connection exists; the `Session`'s
  `:exists()` turns `true` when the server accepts the login, [`SessionAdded`](event/bus/lifecycle.md#sessions)
  fires at that moment and `SessionEnteredWorld` when the character is in the world. The session plays the
  first character the server offers; there is no argument to pick another.
* **One account is one session**, the login screen's own rule.
* **`:forget()` acts on the client only**: a session that account has open stays connected, and `:add(user)`
  refuses until the player logs in again with "Remember me" ticked.

| Call | Result |
|---|---|
| `:add()`/`:forget()` without its permission | raises, naming the permission |
| `:add(user)`/`:forget(user)` with a non-string or empty `user`, or an account not in `:saved()` | raises |
