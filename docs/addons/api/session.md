...
| [`session:fight()`](fight.md) | Its combat schools, its maneuver deck, who it is fighting. |
...
| Rule | Detail |
|---|---|
| Minted once, by identity | `session:world() == session:world()`; a draw callback reading them costs nothing. Each is only ever about one character: a client holding two logins has no *the* world, *the* roster, *the* action bar. The windows among them (a recipe, an action menu) belong to the character that put them up and are usable on a session you tabbed away from. |
| Three are half a namespace | [`session:ui()`](ui/README.md) is the client's own widgets in one character's tree; the windows your addon builds stand in a layer above every session and stay [`hafen.ui():window()`](ui/custom.md). [`session:store()`](store/vars.md) is one character's vars; your addon's own are reached without an address. [`session:console()`](console.md#run-a-line-protected) is one character's command line; the commands your addon registers stay [`hafen.console():on()`](console.md#subscribe). |
...
> **Walking is the whole of what a character you are not looking at will take.** An order to another login carries a destination and never a target: [`session:player():move(position)`](player.md#write-protected) reaches any session; every other write reaches the one on screen and raises for the rest.
...
Nothing here is protected, and a `Session`'s own reads never throw: `session:world()` and `session:player()` answer for a session that has ended, and everything under them reads `nil`-shaped.
...
`:get` addresses, it does not search: a name read out of a [var](store/vars.md) hands back an object before that account logs in and after it goes, and `:exists()` says which. The key is the account only, so a character name hands back a session that does not exist; anything that is not a string raises.
...
| Rule | Detail |
|---|---|
| From any handler | One holding a widget tree included: it names which tree is drawn rather than writing a tree, so a `Draw` handler, a control's press, a hotkey and a console line may write it, and `hafen.session():current()` reads the new session on the line after. The camera and the scene settle on the next frame. |
...
| Rule | Detail |
|---|---|
| Your characters stay logged in | Every session ticks and answers the server behind it. What you log in there arrives as a session like any other, takes the screen because nothing else holds it, and appears in `:list()` with its own [`SessionAdded`](event/bus/lifecycle.md#sessions). Write the screen to a session again to come back. |
...
The [session events](event/bus/lifecycle.md#sessions) report a session connecting, reaching the world, taking the screen or ending, each handing the `Session`. They report changes, not state: an addon loaded while three characters are up hears about none of them, and `hafen.session():list()` is how it learns what is there; a `:reload` announces `SessionEnteredWorld` for every session in the world, the one on screen first.
