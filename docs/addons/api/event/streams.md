...
The client's widgets talk to the server in messages, and these two doors put your addon in the middle. `hafen.event():action()` is for a message on its way out. `hafen.event():message()` is for an update on its way in. Stop, rewrite or re-issue what a widget is about to send. Swallow an update before the widget applies it.
...
`hafen.event():action():on(msg, fn)` fires when a widget is about to send action `msg`, with the arguments fully resolved. For a move `"click"` that is the destination world coordinate, which does not exist at input time. It is the door for stopping or rewriting before the server receives it, where a bus event arrives too late.
...
| Rule | Detail |
|---|---|
| For the handler you are in | A cancel counts only while the client waits for the answer. An `event` stashed and used later raises, as does one whose widget has left the tree. Hold the [widget](../ui/widget.md) and use `widget:send(msg, ...)` outside the moment. |
...
```lua
hafen.event():action():on("click", function(event)
  hafen.log():write("click on " .. tostring(event:gob()))   -- implementation here: act before the click goes through
  event:resend()
end)
```
...
The id is in `event:args()` too, and that copy is not to be read. The wire carries it as a sign-truncated 32-bit number while the server's ids are unsigned. Comparing `args[6]` against `gob:id()` is wrong for every id above 2^31.
...
```lua
local frozen = true                       -- flip it from a hotkey
...
`*` is the key both streams reserve for every message on the stream. It is the one subscription you cannot write by hand, since the list of names is the server's to grow. No message is called `*`.
...
An outbound handler is inside a tree while an inbound one is not. When your Lua is busy on another thread the inbound one waits its turn and the outbound one is skipped, on the terms [threading](../threading.md) states.
...
> **`event:preventDefault()` on an inbound wildcard stops the client.** A wildcard swallows every update, so the widget tree stops receiving from the server. The client's change detection reads applied updates, so [the bus](bus/README.md) stops firing with it. Swallow inside an `if` on `event:msg()`, never at the top of the handler.

| Rule | Detail |
|---|---|
| Where it runs | Where the update arrived, before the widget applies it, which makes `preventDefault` and `rewrite` possible and means the handler cannot be deferred. It holds no character's UI, so it may build a window and write any tree. It is the one row of [threading](../threading.md)'s table that is neither the step nor inside a tree. |
