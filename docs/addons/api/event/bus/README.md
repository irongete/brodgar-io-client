...
| `hafen.event():on("*", fn)` | Throws. Each bus key hands the fact itself: a [Gob](../../gob.md), a [`Meter`](../../meter.md), a [`Session`](../../session.md), a list. A handler for all of them could not name its key. `*` is every message on [a message stream](../streams.md), where the key set is open. |
