...
| List keys | For `StudyChanged`, `EquipChanged`, `KinChanged`, `WoundChanged` the payload is the full new list, not a delta: read the initial state once with the section's own `:list()`, then listen. A list key covers adds, removals and changes (`WoundChanged` fires for a wound appearing, healing and worsening), where a buff arrives one at a time and its edge is named. |
...
| `AchievementUnlocked` | [`Achievement`](../../steam.md#methods-on-achievement) | An achievement is stored as unlocked. |

Both are the Steam client's moments, not a character's: once per client, whichever session is on screen, and not at all on a client started outside Steam.
