...
A game object coming into view and leaving it, what the game or your addon attaches to one, and a click on an entity you put in the world. None carries a session: the world is the client's, so each fires once however many of your characters are looking. Part of [the catalogue](README.md).
...
> **`GobAdded` runs before the object it announces is drawn.** The client holds a newly arrived object out of the scene until every handler has seen it, so a size or an overlay written in the handler is in force on the object's first drawn frame.
...
| Only while somebody is listening | Until an addon subscribes to `GobAdded` nothing is held. The cost begins with the first subscription and ends at the next `:reload`, not at `subscription:off()`: the seam stays armed, and a hold with nobody to hand the object to is released by the next frame. [`GobOverlayAdded`](#overlays-coming-and-going) and [`GobSdtChanged`](#the-state-changing) arm seams of their own. |
...
| Only objects the game sends | A [thing of your own](../../virtual/README.md) (a ghost, a sprite, a model, a standing widget, a patch) fires no `GobAdded` and is drawn the moment you place it. |
...
|---|---|---|
| `event:gob()` | [Gob](../../gob.md) | The gob the overlay is attached to. |
| `event:key()` | `string` | The overlay's key; a resource name for a native one. |
| `event:native()` | `boolean` | `false` for one you attached, `true` for one the game put there (a lit fire's flame). |
...
| `event` | Returns | Description |
|---|---|---|
| `event:gob()` | [Gob](../../gob.md) | The gob whose state changed. |
| `event:sdt()` | `number[]` | The new bytes, the shape `gob:sdt()` answers. |
...
| `event` | Returns | Description |
|---|---|---|
| `event:ghost()` / `:sprite()` / `:object()` / `:patch()` | the [entity](../../virtual/README.md#one-vocabulary-every-kind) | The clicked entity; only the one matching the event reads non-nil. |
| `event:button()` | `number` | `1` left, `3` right. |
| `event:x()`, `event:y()` | `number` | The world point the click resolved to. |
...
| A patch is hit-tested, not picked | Against each of the [pieces](../../virtual/pieces.md) it is the union of, so it answers whether or not you can see that ground (behind a hill, under a house), wherever any piece is under the pointer. The other three are found by the engine's pick pass and answer only where visible. |
