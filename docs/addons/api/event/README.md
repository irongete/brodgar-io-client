...
Subscribe to something the client does instead of polling for it: a client-wide fact on the bus, or a message stream any widget can produce, when there is no widget or control to hold.
...
> Nothing on this bus is polled. Every event fires from the change itself, not from a scan of what differs since the last frame.
