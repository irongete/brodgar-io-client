...
Fetch data from a URL outside the game, protected twice: the `http.get` / `http.post` [permission key](../guides/permissions.md) says whether your addon may use the network, and the manifest's `network` block says which hosts. Pair it with [`hafen.json`](json.md) to read a JSON API.
...
The key says whether, the hosts say where, and the user reads them as one line when they enable your addon: *"fetch data from the servers it lists: api.example.com, \*.githubusercontent.com"*.
...
| `request:on("done", fn)` | [`Sub`](event/README.md#subscribe) | Unprotected | The handler, called once with the [result](#the-result-object); legal before and after `:send()`. |
...
| Rule | Detail |
|---|---|
| The URL is parsed strictly | By RFC 3986: a space, an unescaped `{`, `}`, `\|` or `^`, a malformed `%` escape, no scheme, or a scheme other than `http`/`https` raises at `:request(url)` naming the URL. The parsed host is the one the allowlist is asked about. Percent-encode what you interpolate into a path or query. |
| Asynchronous | `:send()` returns at once; the handler runs a frame or more later on the [step](threading.md) of the character whose login sent it, holding no widget tree, so it may read and write any. There is no blocking form. |
...
| Headers ignored | Transport-owned ones (`Host`, `Content-Length`, `Connection`, `User-Agent`, …), `Accept-Encoding` (the client asks for an uncompressed body, what the 8 MB cap is measured against) and `Cookie` (a session the client does not keep). |
