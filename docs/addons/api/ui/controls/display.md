...
A label, a picture, a separator and a progress bar: the [controls](README.md) with nothing to click. Each is built bare, configured by chained setters and dressed by the [stylesheet](../style/README.md).
...
```lua
local stamina_label = hafen.ui():label():text("")
local percent = 72
