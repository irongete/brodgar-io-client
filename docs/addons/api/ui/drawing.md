...
Every draw callback — a widget's [`Draw`](custom.md), an [overlay over the screen or a widget](overlay.md), a [`gob:overlay()`](../overlay.md) painter — receives `g`, a drawing surface whose coordinates are the callback's own local pixel space.
...
| Method | Description |
|---|---|
| `g:text(str, x, y [, opts])` | Text with its top-left at `(x, y)`; `opts` is `{font, color, width}` — [text](#text). |
| `g:atext(str, x, y, ax, ay [, opts])` | Anchored text: `ax`/`ay` in `0..1` pick which point of the text sits at `(x, y)`. |
| `g:rect(x, y, w, h)` | A rectangle outline one screen pixel wide, whatever the scale, tracing a design-pixel box. |
| `g:frect(x, y, w, h)` | A filled rectangle. |
| `g:line(x1, y1, x2, y2 [, width])` | A line; `width` in design pixels, default `1`, so a `4` px rule keeps the chrome's weight on a scaled client. |
| `g:poly(x1, y1, x2, y2, x3, y3, ...)` | A filled convex polygon of three or more points in the current colour. The one verb not clipped to the widget's box — [below](#clipping). |
| `g:prect(cx, cy, radius, fraction)` | A clockwise pie or progress wedge, `fraction` in `0..1`. |
| `g:image(img, x, y [, w, h])` | An [image asset](../asset/README.md) at native size, or scaled into `w × h`. |
| `g:aimage(img, x, y, ax, ay)` | An image asset anchored, as `g:atext`. |
| `g:resource(name, x, y [, w, h])` | A client `.res` image by name, at native size or scaled — [images](#images). |
| `g:color(r, g, b [, a])`, `g:color(c)` | The draw colour, `0..255` components or a [colour table](../shapes.md#colours); `g:color()` resets to white. |
...
| Outlines | An [outline](../font.md#an-outline-round-every-glyph) is part of the face, so a variant carrying one and the face it derives from are two entries, each costing one entry and one blit. |
...
| Overlay labels | [`ov:text(s)`](overlay.md#a-label-the-client-draws) over a widget and [the same verb at a gob](../overlay.md) are drawn by the client through this cache on the same key, at no Lua cost per frame. |
