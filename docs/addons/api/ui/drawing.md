...
win:on("Draw", function(event)
  local graphics, width = event:g(), event:w()
...
hafen.ui():overlay():add("meters"):draw(function(graphics, width, height)
  ...
  graphics:color(255, 200, 0)                 -- components, like every other g: call
  graphics:frect(4, 4, 40, 6)
  graphics:color{255, 200, 0}                 -- the same colour, as a table
  graphics:frect(4, 14, 40, 6)
  if health then
    graphics:color(health:color())                -- ...and a colour read back from the API, unchanged
...
hafen.ui():overlay():add("icons"):draw(function(graphics, width, height)
  if icon then
    ...
    graphics:image(icon, 4, 40, 16, 16)               -- the same image scaled to 16x16
    graphics:aimage(icon, width - 6, 6, 1.0, 0.0)         -- anchored to the screen's top-right corner
  end
end)
```
