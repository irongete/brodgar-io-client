...
| Secret fields | A password entry and the Kin window's hearth secret answer `nil` to `:text()`, `:value()` and `:info()`, however reached; no permission reads them. Writing one is `:value(v)` under `widget.value`. The [Kin window](../kin.md#the-row-is-where-the-walk-ends) also stops a `:parent()` walk from inside. |
...
| Effect | Detail |
|---|---|
| Drawn dimmed | It and everything under it: a control's caption and art, a surface's [stock](custom.md#naming-and-dressing-your-own-surfaces), every colour a `Draw` handler names. A button and a checkbox also wear the `disabled` [face](style/chrome.md#a-face-per-state) a rule names on [`button`](style/surfaces.md#button) or [`checkbox`](style/surfaces.md#checkbox-scrollbar-and-slider). |
...
| No key reaches it | A disabled entry loses the keyboard at once, Tab passes over it, and a hotkey a widget under it would answer goes on to the rest of the screen. |
