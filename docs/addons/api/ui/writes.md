...
local inventory = hafen.session():current():ui():inventory()      -- borrowed: the reads, and a few writes
inventory:position(40, 40)                     -- a level over the client's own place, and it restores
win:enabled(false)                       -- greyed out, whole: no press, no key, dimmed
inventory:enabled(false)                       -- error: its state is the client's
```
...
advanced_switch:on("Changed", function(checked) group:enabled(checked) end)       -- the switch outside it brings it back
```
