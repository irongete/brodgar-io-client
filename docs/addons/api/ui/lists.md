...
```lua
local list = hafen.ui():list():size(200, 160):rows{"Wood", "Stone", "Clay"}
list:on("Changed", function(row) hafen.log():write("picked " .. row) end)
```
...
`hafen.ui():dropdown()` answers the same `:value()`/`:value(v)`/`Changed` as a list — the picked row,
