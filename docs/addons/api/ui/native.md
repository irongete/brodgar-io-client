...
|---|---|---|---|
| `widget:position(x, y)` | `self` | Unprotected | Moves it, writing the field the user's own drag writes: what you place is what you click. On the HUD or in the layer the place follows the screen (*A place on the screen follows the screen*, below). |
| `widget:size(w, h)` | `self` | Unprotected | Resizes its content. A window's chrome refits. |
| `widget:position(nil)`, `widget:size(nil)` | `self` | Unprotected | Drop your level of that half. The widget lands on a rule that still names it, else on the stock: what the client recorded on your first touch, or the builder's default place and box on a [surface of yours](custom.md). A window on the HUD or in the layer that follows the screen lands at the user's place for the size the screen is now (*Re-layout*, below). |
...
| A drag writes your `:position` level | `:position()` reads where the user dropped it. `:position(nil)`, `:reload` and disable put the stock place back. The client's own store still records what the user placed. On a widget directly on the HUD or in the layer the level is the user's place, so it follows a resize of the screen (*Re-layout*). Your own `:position(x, y)` over it follows the same way, from where it lands. |
...
| Re-layout | Resizing the game window re-places the client's panels, and a window directly on the HUD or in the layer keeps its place relative to the screen: its centre stays at the same fraction of the screen's width and height. A window within 10 design pixels of an edge, or left partly past it, stays on that edge, and a window that fits is always kept whole on screen. A client window follows. A window your addon built follows once something has placed it: your `:position(x, y)`, or the user dragging it by its title. Any level on a widget directly on the HUD or in the layer follows too, whoever placed it: your `:position(x, y)`, a `:draggable` drag, the title-bar drag of a window your level holds, a place `:remember` put back. A rule's `position` keeps its pixels, since a rule says a place that follows the screen with an [anchor](style/geometry.md#anchor), and `:position(nil)` lands a client window at the user's relative place. Anything you [anchored](style/geometry.md#anchor) to a window that moved follows it on the next step. |
