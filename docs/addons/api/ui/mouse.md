...
mouse:x()  mouse:y()                  -- where the cursor is, in root coords (design pixels)
mouse:over()                      -- the deepest Widget under it, or nil
mouse:shift() mouse:ctrl() mouse:alt()    -- the live modifier keys
mouse:cursor("hand")              -- the pointer's picture, while a mode of yours wants a click
mouse:pick()                      -- the OBJECT in the world under it, or nil
mouse:ground()                    -- ...and the GROUND under it, as a Position
```
...
```lua
local mouse = hafen.ui():mouse()
mouse:cursor("hand")                    -- ...the user now sees a hand wherever they point
mouse:cursor()                          -- "hand"
mouse:cursor(nil)                       -- and back to whatever the client would have drawn
```
...
mouse:pick()                                               -- ...and the same answer, read on demand
mouse:ground()                                             -- the place under the pointer, a Position
...
```lua
local grab = hafen.ui():mouse():grab()   -- bare: from here the pointer is yours

grab:on("Move", function(event) end)        -- event:x() event:y() in root coords; event:shift() event:ctrl() event:alt()
grab:on("Up",   function(event) end)        -- …plus event:button(); fires once and auto-releases

grab:release()                           -- hand it back early
```
...
grab:on("Move", function(event)
  ...
  world:screenToWorld({x = event:x(), y = event:y()}, function(position)    -- position is a Position, a frame later
