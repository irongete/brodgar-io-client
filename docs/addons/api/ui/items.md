...
An item is a thing the client draws, found through the icon drawing it: `widget:item()` on the icon, `widget:items()` on the widget around it — a backpack's cells, the cursor, a recipe's slots, the food icon on a constipation row — all the same [`Item`](#the-item-object) object.
...
| `widget:item()` | `Item \| nil` | Unprotected | The item one icon draws: a container's cell, the cursor, a recipe slot, a constipation row's food icon, a listing a resource ships its own widget for. `nil` on every other widget; the [`item` role](selectors.md#roles) is the same test. |
...
|---|---|---|
| `item:res()` | `string \| nil` | Resource name, the item's stable identity. |
| `item:name()` | `string \| nil` | Display name, once the tooltip has resolved. |
| `item:quantity()` | `number \| nil` | [The count on the icon](#the-two-numbers-on-an-icon); `nil` for one showing none. |
| `item:progress()` | `number \| nil` | [The arc over the icon](#the-two-numbers-on-an-icon), `0..1`; `nil` for one painting none. |
| `item:durability()` | `{cur, max} \| nil` | [The two counts its wear row prints](#durability-the-counts-a-wear-row-prints). |
| `item:quality()` | `number \| nil` | The quality the tooltip shows. |
| `item:contents()` | [`Contents`](contents.md) `\| nil` | What it holds; `nil` for an item holding nothing. |
| `item:container()` | `Item \| nil` | The item it sits inside; `nil` for one in a container widget. |
| `item:cell()` | `{x, y} \| nil` | The 1-based grid cell it sits in — a place, where a grid control's [`:cellSize()`](widget.md#read-methods) is a size. |
| `item:slots()` | `string[]` | The equipment slots it fills, by name; empty for anything not worn. |
| `item:handle()` | `number \| nil` | Its server widget id, the number it is addressed by on the wire; `nil` once gone. |
| `item:exists()` | `boolean` | Whether the icon drawing it is still in the tree. |
| `item:info()` | `table` | The [snapshot](../types/items.md#item): every read above in one table. |
| `item:on("Changed", fn)` | [`Sub`](../event/README.md#subscribe) | [What the item says about itself resolved or was revised](#an-item-arrives-before-it-can-be-described). |
...
A depiction is painted from a resource with nothing on the wire behind it: a recipe slot, a price on a listing, an icon resource code puts up. It is an `Item` reading `:res()`, `:name()`, `:quality()`, `:durability()` and `:info()` off the same tooltip mechanism, and it answers nothing about where it is.
...
The server sends the icon first and the tooltip after, and the code that reads a quality, a wear row or a contents block out of it ships in a resource that may still be loading. Until both landed, `:name()`, `:quality()`, `:durability()`, `:contents()`, `:quantity()` and `:progress()` answer `nil`. `item:on("Changed", fn)` fires when the tooltip resolves, and again whenever the server revises it.
...
| A recipe slot is described on arrival | The client works out what a slot names while building the icon, so it arrives with `:name()` and `:quality()` answering and nothing fires behind them: read first, subscribe second. |
...
An icon wears up to a count in the corner and an arc round the middle, each with two sources — a server-written field and the item's own tooltip — folded in the same order by `:quantity()` and `:progress()`: if it is on the icon, the verb answers it; an item showing neither, or with an unresolved tooltip, answers `nil` to both.
