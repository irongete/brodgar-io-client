...
inside it, [controls](controls.md) puts the client's own buttons in it instead of painting them, and
[lists](lists.md) does the same for a scrolling list of rows.
...
| [lists](lists.md) | a scrolling list of rows, and the row source it shares with a radio |
