...
hafen.ui():hit(mouse:x(), mouse:y()) == mouse:over()          -- true: one pair, asked two ways
```
...
```lua
local mouse = hafen.ui():mouse()
local world = hafen.session():current():world()      -- a pixel is the screen's
...
win:on("Draw", function(event)
  event:g():rect(0, 0, event:w(), event:h())              -- on the edge of the box :size(w, h) gave it
end)
```
...
session:ui():match("window[title=Equipment]"):position()   -- {x = 40, y = 200}, at any scale
```
