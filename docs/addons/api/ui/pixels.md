...
Every coordinate and size a widget takes or gives is a design pixel. That is one pixel before the client applies the user's interface scale, the unit the client's own layout and art are written in.
...
| The client's art | Authored several times larger and resampled once per scale. A `{res = …}` in a `bg` or `border` is measured and sliced in the same unit. It is crisper than a file of that apparent size on a scaled-up client ([naming a picture](style/chrome.md#naming-a-picture)). |
...
> The inverse conversion is not exact. A widget the client placed does not generally sit on a whole design pixel. Reading a client widget's position and writing the same pair back may shift it by less than one design pixel. Hand a place back with [`:position(nil)`](native.md), not by rewriting the numbers.
