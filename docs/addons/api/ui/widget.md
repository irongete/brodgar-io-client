...
if inventory then hafen.log():write(inventory:type() .. " holds " .. inventory:items():count() .. " items") end
```
...
hafen.ui():hit(mouse:x(), mouse:y()) == hafen.ui():hit(mouse:x(), mouse:y())   -- true
session:ui():inventory() == session:ui():node(invId)                     -- true: one widget, one object
```
...
| `:size()` | `{w=, h=}` | size, in [design pixels](pixels.md): the box `:size(w, h)` writes, which on a window is its **content** area; the frame's own box is `:chrome().frame`. `.x` on one [raises](../shapes.md#the-anonymous-shapes) |
...
| `:rootPos()` | `{x=, y=}` \| nil | its top-left in **root coords**; with `:size()` that is the rectangle outlining it — on a window, with `:chrome().frame` |
| `:replacement()` | Widget \| nil | the view **you** put in place of this widget's window, or `nil` — see [replace](replace.md) |
| `:chrome()` | table \| nil | on a **window**, where its decoration drew its [ornaments](style/chrome.md#ornaments) — `{frame = {w=, h=}, content = {x=, y=, w=, h=}, caption = {x=, y=}, plate = {x=, y=, w=, h=, styled=}, sizer = {x=, y=}, close = {x=, y=, w=, h=}}`, `frame` (the outer box) and `content` (the area inside it where the children stand, from the frame's top-left) always, each ornament only once it has been drawn; `nil` on anything else |
...
local from = hafen.ui():tipAt(mouse:x(), mouse:y())        -- who speaks for this point, or nil
