...
A column lays its children out top to bottom and a row left to right. They go in tree order, a gap apart, the cascade's `padding` in from the edge, each child's `margin` around it. Its box is what they take.
...
| Refused children | A window: the user places it, and its chrome measures itself without telling the column. A client widget: its place is the client's, so take it into a bare widget of yours and put that in the column. Both refusals say why. |
...
| When it re-lays | On the events that change it, before the call returns. A child entering, resizing, hiding, showing or leaving. A `:gap`, `:size` or `padding` written on the column. A `margin` written on a child. Never per frame. |
...
A column is as wide as its widest visible child and as tall as all of them, gaps, padding and margins included. An empty one is `0` by `0`.

| Method | Returns | Permission | Description |
|---|---|---|---|
| `column:size()` | `{w=, h=}` | Unprotected | Reads the box. |
| `column:size(w)` | `self` | Unprotected | Pins the width. The height follows the children. The one-number arity a [control's art](controls/README.md#sizing) earns it means the same here. |
| `column:size(w, h)` | `self` | Unprotected | Pins both. Children past the box are clipped. |
| `column:size(nil)` | `self` | Unprotected | Both follow the content again. |
| `column:pack()` | — | Unprotected | Refused: a column is packed by construction. A window around a column still packs — [`window:pack()`](custom.md#packing-a-surface-around-what-is-inside-it) sizes the window to the column and follows it from then on. |
...
`padding` is the room between the column's edge and its children. The first child starts at the left and top inset. The box ends one right and bottom inset after the last. It is the same [`padding`](style/chrome.md#padding) a window frame keeps, resolved by the [cascade](style/README.md#the-cascade). That is a tree rule naming the column, its own `:stock` beneath every rule, or `widget:rule()` above them.
...
`margin` is the room the column keeps around one child: four insets outside that child's box. It is a property of the same cascade said about the child, by a tree rule naming it, its own `:stock`, or `widget:rule()` on it. [Geometry](style/geometry.md#margin) is the property's page.
...
| Rule | Detail |
|---|---|
| Added to the gap, never collapsed | The child sits its left and top inset further in. The next child starts after its bottom inset and the gap. Two margins meeting across a gap are both kept. `:position()` on a child is that arithmetic. The sum is made in the client's own pixels. A total added up from controls' `:size()` reads can differ from the read by one on a scaled client. Bare widgets you sized add up exactly. |
| Across the axis | The column is as wide as its widest child with that child's left and right insets. A right inset is room the box keeps. A left one is an indent for that row alone. |
...
A window packed around a column. In it a row (icon and checkbox) and a switch. Then the group the switch governs, indented by a left padding and disabled until the switch is ticked. Then a scroll holding a column of rows.
