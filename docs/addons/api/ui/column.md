...
local advanced_switch    = hafen.ui():check():parent(panel):text("Advanced")  -- the switch, outside the group
...
local scroll_box   = hafen.ui():scroll():parent(panel):size(160, 120)    -- a fixed box in the column...
local list = hafen.ui():column():parent(scroll_box):position(0, 0)       -- ...and a column that outgrows it
