...
The [sheet](README.md) properties that paint a surface (`bg`, `border`), move the client's content (`padding`), replace a plate (`picture`) and dress a window's ornaments (`caption`, `sizer`, `closeButton`). [The key table](keys.md#what-each-key-accepts) says which surface does what with which.
...
|---|---|---|
| `rule:bg(t)` | One [surface](#naming-a-picture), or an array of them, each with a [face per state](#a-face-per-state) | What something is painted on: one layer or several, in paint order — [bg](#bg). |
| `rule:border(t)` | `{<art>, slice = {l, t, r, b}}`, `{box = "gfx/hud/wnd"}` or `{color = {r, g, b[, a]}, width = n}` | The frame around it: your own 9-slice, one of the client's, or a line — [border](#border). |
| `rule:padding(n)`, `rule:padding(l, t, r, b)` | Design pixels, `>= 0` | The room between a frame and its content — [padding](#padding). |
| `rule:picture(t)` | One surface, with a face per state | The whole plate a surface is — [picture](#picture). |
| `rule:caption(t)` | `{at =, offset =}` | Where a window's title is measured from — [ornaments](#ornaments). |
| `rule:sizer(t)` | A surface with `at` | The corner grip a resizable window draws, and where. |
| `rule:closeButton(t)` | A surface with `hover`, `pressed`, `at`, `offset` | The window's close button: its face and its corner. |

---

## Naming a picture

Wherever a picture may go it is one value with four spellings, one of them the client's own art, so nothing has to be extracted from the game's resources.

| Written | Is |
|---|---|
| `{color = {r, g, b[, a]}}` | A flat fill, alpha included; takes none of the three fields below. |
...
One surface, or an array painted in order: a texture under a vignette, or the client's own window background, which is a tiled field then a shade down each side.
...
Beside its own value a `bg` or a [`picture`](#picture) may name one for `hover`, `pressed`, `disabled` or `checked`: a whole value of the same shape, worn while the surface is in that state, falling back to the value it sits in.
...
A state rides inside the value, never in the selector: the surface drawing itself knows its state, so no hover key exists. Which states a surface enters is [the surface's own](keys.md#what-each-key-accepts): a button three, a checkbox `checked`, a window frame none; a face nothing asks for costs nothing.
...
| `parts` | An array of pieces pinned inside the frame, each a [surface](#naming-a-picture) with a required `at`, painted over the frame in order (the client's window drops one at the foot of its left edge). An empty list raises. |
...
`bg` is what a surface is painted on; `picture` is what a surface is — the whole plate, for the places the client blits an image rather than framing something: the [HUD's own plates](hud.md), `menu.frame`, `meter`, and every picture the server places in a window.
