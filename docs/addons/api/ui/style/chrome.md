...
sheet:rule("window.frame"):padding(8, 24, 8, 8)  -- ...or room for a title bar at the top alone
sheet:install()
```
...
sheet:rule("@Img"):picture{ res = "gfx/hud/brframe" }               -- ...and the ones the server places
sheet:install()
```
...
if window then hafen.log():write("caption at " .. window:chrome().caption.x) end
```
