...
A channel answers the key its lines resolve at as [`channel:kind()`](../../chat.md#the-kinds), and a line its own as [`msg:kind()`](../../chat.md#the-kind-of-a-line). `chat.mine` is the one key a line carries and no channel does. A rule that colours a kind and an addon that reads one name the same thing.
...
## The colours the client walks

`chat.speaker` is a colour per speaker and `chat.urgent` a colour per urgency level. A single colour cannot express either, so both take a sequence and refuse a colour.
