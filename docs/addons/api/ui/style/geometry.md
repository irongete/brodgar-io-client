...
- **They are said about a widget, so only a [tree key](keys.md#tree-keys) may carry them.** `["chat"]`,
  ...
  position of its own to move, so `pos` or `size` there is an **error** naming the fix rather than a rule
...
An absolute `pos` is a number you wrote down once. **An anchor is a relationship**, re-derived every time
