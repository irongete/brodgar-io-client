...
```lua
local sheet = hafen.ui():sheet()
local window = hafen.session():current():ui():match("window[title=Equipment]")
sheet:rule("window[title=Equipment]"):position(40, 200)
sheet:install()
window:position()            --> {x = 40,  y = 200}    -- the rule
window:position(12, 12)      --                        -- named by hand: the top of the same fold
window:position()            --> {x = 12,  y = 12}
window:position(nil)         --                        -- your level goes...
window:position()            --> {x = 40,  y = 200}    -- ...and the RULE is what is underneath
...
sheet:rule("window[title=Equipment]"):anchor{ to = cur:ui():match("window[title=Inventory]"),
                                          at = "topright" }
