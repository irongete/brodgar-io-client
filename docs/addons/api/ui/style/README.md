...
sheet:rule("tooltip"):color{255, 150, 90}   -- ...so this lands on the spot
sheet:rule("chat"):release()                -- ...and so does giving one rule back
...
```lua
local hovered = hafen.ui():mouse():over()      -- the widget under the cursor
...
hovered:rule():release()                       -- give the level back
```
