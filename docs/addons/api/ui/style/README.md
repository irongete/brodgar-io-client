...
One sheet per addon says what the client looks like. A [selector](../selectors.md) names a rule, and the rule's properties are setters. Installing the sheet applies them live. To draw your own pixels, use [custom](../custom.md) and [drawing](../drawing.md). Nothing here is protected.
...
`sheet:load(rules)` takes a table of `["selector"] = {property = value}`, the entry point for a look kept in a file.
...
| Rule | Detail |
|---|---|
| Every value has a file spelling | A colour is a table of numbers. A border is a slice of four numbers or a colour and a width. A `padding` is one or four numbers. An [anchor](geometry.md#anchor) is a corner and an offset. A picture is a [path or a resource](chrome.md#naming-a-picture). A face is `{builtin = "mono"}` or `{asset = "fonts/Inter.ttf"}` — [text](text.md#font). No handle anywhere. |
| Rules are in selector order | A keyed Lua table has no order, so a loaded sheet's rules are sorted by selector. That is the order `sheet:info()` reports and the [equal-specificity tie-break](keys.md) uses, the same on every run. Rules that must be ordered are written with `sheet:rule(sel)`, applied in call order. |
...
`sheet:stock()` is what this client draws with when no rule says anything, keyed by [site](keys.md#site-keys), in the shape `sheet:load(rules)` takes. Art is named by resource, faces by built-in, colours as the [keyed table](../../shapes.md#colours) every reader hands back.
...
| Only what it can say whole | A surface made of things the vocabulary has no word for is left out. That is a fill inset inside its end caps, a chain of links spread down a bar, a corner following its caption. So what you load back paints the client you saw. |
...
| Walked colours | `chat.speaker` and `chat.urgent` come back as the sequences they are — [the chat](chat.md#the-colours-the-client-walks). |
...
|---|---|---|---|
| `rule:font(face)` | A [font handle](../../font.md), or the face named: `{builtin = …}` / `{asset = …}` with `size`, `bold`, `italic`, `aa` | Unprotected | [text](text.md#font). |
| `rule:color(c)` | A [colour](../../shapes.md#colours), `{200, 210, 220}` or `{r=, g=, b=[, a=]}`. On `chat.speaker` and `chat.urgent` the sequence instead | Unprotected | [text](text.md#color). |
| `rule:emboss(v)` | `false`, or `{texture = <art>}` | Unprotected | Whether the client's relief is cut through a surface's letters, and with what. `false` lets `color` reach a caption — [text](text.md#emboss). |
| `rule:glow(t)` | `{color = {r,g,b[,a]}, radius = n}` | Unprotected | The halo behind a carved surface's letters. Radius `0` is none — [text](text.md#glow). |
| `rule:bg(t)` | One [surface](chrome.md#naming-a-picture), or an array | Unprotected | What something is painted on — [chrome](chrome.md). |
| `rule:border(t)` | `{<art>, slice = {l, t, r, b}}`, `{box = "gfx/hud/wnd"}`, or `{color = …, width = n}` | Unprotected | A 9-slice frame, one of the client's, or a line — [chrome](chrome.md#border). |
| `rule:padding(n)`, `rule:padding(l, t, r, b)` | Design pixels, `>= 0` | Unprotected | The room between a frame and its content — [chrome](chrome.md#padding). |
| `rule:margin(n)`, `rule:margin(l, t, r, b)` | Design pixels, `>= 0` | Unprotected | The room a [column](../column.md) keeps around the matched widget. Tree keys and `widget:rule()` — [geometry](geometry.md#margin). |
| `rule:picture(t)` | One surface, a face per state | Unprotected | The whole plate a surface is — [chrome](chrome.md#picture). |
| `rule:caption(t)` | `{at =, offset =}` | Unprotected | Where a window's title is drawn — [ornaments](chrome.md#ornaments). |
| `rule:sizer(t)` | A surface with `at` | Unprotected | The corner grip a resizable window draws — [ornaments](chrome.md#ornaments). |
| `rule:closeButton(t)` | A surface with `hover`, `pressed`, `at`, `offset` | Unprotected | The button that closes a window — [ornaments](chrome.md#ornaments). |
| `rule:position(x, y)` | Design pixels | Unprotected | Where the widget sits in its parent. Tree keys only — [geometry](geometry.md). |
| `rule:anchor(t)` | `{to =, at =, offset =}` | Unprotected | The same place as a relationship — [geometry](geometry.md#anchor). |
| `rule:size(w, h)` | Design pixels | Unprotected | How big. A window's content box. Tree keys only — [geometry](geometry.md). |
| `rule:selector()` | `string \| nil` | Unprotected | The key it was named by. `nil` on a widget's own level. |
| `rule:sheet()` | `Sheet \| nil` | Unprotected | The sheet it belongs to. `nil` on a widget's own level. |
| `rule:release()` | `self` | Unprotected | The level stops saying anything. The handle stays usable, and a setter says the level again. |
| `rule:info()` | `table \| nil` | Unprotected | Every property it sets plus its `selector`, or `nil` when it says nothing. The `selector` is not itself a load value: take the key out before `sheet:load`. |
...
| `:style()` is the result | A style inherited from an enclosing widget is applied at the draw, not resolved onto the child. A child inside a styled window reads `nil`. Site keys are not folded in either. |
...
| Owned and weak | Tagged with your addon, reverted on `:reload` and disable, held weakly against the widget. It goes when the window closes. A stashed rule then reads `nil` and writes as a no-op. |
...
| The bottom two levels | The client's surfaces have a stock look a rule beats. A widget an addon built may declare one the same way. That is why a default look is a [stock](../custom.md#naming-and-dressing-your-own-surfaces) and not a rule of the addon's own. |
...
The sheet reaches any render site ([site keys](keys.md#site-keys)), any widget a [selector](../selectors.md) names, and any single widget through `widget:rule()`. It says the text and the surfaces that paint. It says the room inside a frame and around a row, the plate a surface is, and a window's ornaments. It says where a widget is and how big. Each is per property, live, owned, reversible and loadable from a file. What it does not reach:
...
| A state in the selector | No hover, pressed, focused or disabled key: a rule matches what a widget is. A state is part of the value — a [`bg`](chrome.md#a-face-per-state) or [`picture`](chrome.md#picture) names a face per state. Your own widgets draw states in `Draw`. |
...
| A configuration UI | The client ships the mechanism (layout is data, `widget:position()` reads it, [`hafen.store`](../../store/README.md) keeps it). An addon ships the editor. |
