...
A **var** — a saved variable — is a Lua table you name at `var`, saved for you: it exists the first time
`var(name)` names it, the client fills it from your [file](README.md) then and there, and writes it back
when it changes. You assign into it, and that is the whole of saving. It is the shape for settings — a
handful of values read once and changed now and then; a record that grows is a [table](tables.md). Every
verb here is **unprotected**.

**The door is the scope.** `hafen.store()` reaches your addon's own vars: one set of rows for the
whole client, whichever account or character is up. A character's vars are that character's own
...
[raises](#each-characters-vars-are-their-own).

## Read and write

| Method | Description |
|---|---|
| `s:store():var(name)` | the live table of that character's var `name`, empty until something is saved under it |
| `s:store():list()` | the names that exist in that character's scope, sorted, as a string array |
| `s:store():flush()` | write that character's changed vars now; the store |
| `hafen.store():var(name)` | the live table of your addon's own var `name`, empty until something is saved under it |
| `hafen.store():list()` | the names that exist in your addon's own scope, sorted, as a string array |
| `hafen.store():flush()` | write your addon's own changed vars now; the store |

**What `var` hands back is the table itself, not a copy**, so writing into it is the whole of saving: there
...
**A name is what you call the var, and there is nothing to hold it against.** So a misspelt name is
an empty var, as a misspelt table name is an empty table, and the same name through both doors is
two vars that never meet: a value written through one is not in the other. A var nobody names
...
`list()` answers for its own scope — a var written and not yet flushed is on it, and nothing of the
other scope is.

## Each character's vars are their own
...
Two states have no vars to answer with, and both **raise** rather than hand back an empty table that
...
**When each scope is ready.** Your addon's own vars are readable in the file body and in `Load`. A
...
A var is stored as JSON, so tables, strings, numbers and booleans round-trip and nothing else does —
...
**A cycle is one of the things a var cannot hold**, and `flush()` names it like any other: a table
that contains itself has no JSON spelling, and the forgiving writes put the literal text `"<cycle>"` where
it was, which reads back as text. So do the **keys**: a var is keyed by strings and numbers, and a
...
the path of the first value a var cannot hold instead of writing it, and it refuses first of all when
...
so a var that quietly turned into text is reported whether or not you ever call `flush()`.
...
something that is **not** a var — a single value, `null` — is the same failure and is reported the same
...
sits and how big it is, with no var and no code of yours. That is deliberate: a placement is saved by
...
**It is filed under the tree the widget stands in**, the way a var is filed under its door:
...
saves though it names no var at all, and a place you wrote yourself with `w:position(x, y)` lands
with the widget.
