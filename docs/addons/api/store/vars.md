...
A var is a Lua table you name at `var`, filled from your [file](README.md) the first time it is named and written back when it changes: assigning into it is the whole of saving. The shape for settings; a record that grows is a [table](tables.md). Unprotected.
...
The same name through the two doors is two vars. A character's key is named by the server (the world's name and the character's, reduced to letters, digits, `.` and `-`, joined by `_`): one character is one key in every file, and one who has not reached the world has none yet.
...
| Two states raise | A session that is not live (its rows were written when it ended; `session:exists()` tells) and one that has not reached the world (no character, no key; `SessionEnteredWorld` is when that ends). Neither hands back an empty table that would take writes and never save them. |
...
| A row that cannot be read or parsed | Leaves the table as it is, logged not raised: empty settings rather than an addon that does not start, and that scope [read-only for the session](README.md#when-the-file-cannot-be-opened). A row that parses into something that is not a var (a single value, `null`) is the same failure. |

## Where a widget sits is saved for you

[`widget:remember(name)`](../ui/native.md#remembering-where-the-user-put-it-unprotected) keeps where a widget sits and how big it is, with no var and no code of yours: written the moment the gesture lands, under the name you gave once, as a row of the client's own file `savedata/client.sqlite`, keyed by your addon and never in yours.
...
A session's own window has nothing to put back until that session is in world, and tabbing moves nothing. No `flush()` and no timer writes a placement: the row is written when a drag or a resize lands, when the screen changes and when the widget goes (destroyed, closed, torn down with your addon). A place you wrote with `widget:position(x, y)` lands with the widget.
