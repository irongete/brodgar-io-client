...
|---|---|---|---|
| a **var** | a live Lua table you name at `var` and assign into, written for you | settings: a handful of values you read at load and change now and then | [vars](vars.md) |
...
**Which is which.** A var is whole: it is held in memory as one table and written as one row, so it
is right for what you would keep in a settings table and wrong for what grows — ten thousand map nodes in
a var are ten thousand entries serialised at every save. A table is rows: each `:put` is one row in
...
one character's you say so: a var reached through a character's [session](../session.md) is a row
...
or character is up. A character's vars are the one thing reached elsewhere, through
[`s:store()`](vars.md#read-and-write).

| Verb | Answers | Page |
|---|---|---|
| `hafen.store():var(name)` | the live table of your addon's own var `name`, empty until something is saved under it | [vars](vars.md) |
| `hafen.store():list()` | the names that exist in your addon's own scope, sorted, a string array | [vars](vars.md) |
| `hafen.store():flush()` | the store, once your addon's own changed vars are written | [vars](vars.md) |
...
names a var, so that a var is readable in `Load` and a table is there to declare. Two addons
...
| a [remembered placement](vars.md#where-a-widget-sits-is-saved-for-you) | never: it is a row of the client's own file, not of yours, written when the gesture lands, when the screen changes and when the widget goes — no `:flush()` writes it |
...
vars are written. A crash or a kill closes nothing: every row a call committed is in the log and is
folded in at the next open, while a var's changes since its last write are gone. A power cut can
...
2. **No extension loads.** `load_extension` is switched off, so the functions a statement has are SQLite's
   own.
3. **The `hafen_` prefix is the client's, and it names one table.** `hafen_vars` holds your vars,
   reached through `:var(name)` and never through a statement: a statement or a declaration naming a
   `hafen_` table is refused naming it. Your [remembered placements](vars.md#where-a-widget-sits-is-saved-for-you)
...
log line names the file and the cause, your vars are empty and are never written, and every other
verb here refuses naming the cause. Fix it and `:reload`.

> **A scope whose row could not be read is read-only for the session.** A var's row that is there and
...
> is simply **not there yet** is not this case: an empty var is the whole truth, and the first write
> creates it.

## See also

- [vars](vars.md) — the two doors as the two scopes, what survives, when it is written, and where a widget sits
