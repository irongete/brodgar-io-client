...
Everything your addon stores between sessions lives in one SQLite file, `savedata/<id>/<id>.sqlite`, read and written by every account this client logs in with and every character they play; `hafen.store()` is that file. Every verb here and on the three pages under it is unprotected: nothing reaches the server.
...
A character is a column or a row key, never a file: a var reached through a character's [session](../session.md) is a row keyed by that character; a table's rows are one character's where you declare a column for it. The whole picture is one `SELECT` away, because it is one file.
...
| Method | Detail |
|---|---|
| `hafen.store():info()` | `file` is the path of `savedata/<id>/<id>.sqlite`; `bytes` the size of the database, its pages: what is committed to the log as well as what is in the file, and the free pages a `:vacuum()` would give back. The one snapshot the store hands out. Raises when the file is [unavailable](#when-the-file-cannot-be-opened). |
...
| Rule | Detail |
|---|---|
| One file per addon | Named by the id, in a folder named by the id; no path of your choosing, no file per account or character (a file per login is one schema in as many files as logins, which no query reads together). Created the first time your addon loads, so a var is readable in `Load` and a table is there to declare. Two addons never share one. |
...
An open that fails (a folder that cannot be written, a file that is not a database, a lock another process holds, a runtime without the driver) leaves the store unavailable and your addon loaded: one log line names the file and the cause, your vars are empty and never written, and every other verb refuses naming the cause. Fix it and `:reload`.

> **A scope whose row could not be read is read-only for the session.** A var's row that is there and will not parse leaves that scope's tables empty, and a write would replace the only copy of your data with that empty set: the timer, `:flush()` and the teardown skip it, and the log names the row. A load that succeeds lifts it (a `:reload` or the next launch once the row is readable). A row not there yet is not this case: an empty var is the whole truth, and the first write creates it.

---

## See Also

- [Vars](vars.md) — the two scopes, what survives, when it is written, and where a widget sits.
