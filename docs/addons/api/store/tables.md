...
A table you declare is your addon's record: columns, a key and indexes in your [file](README.md). Its rows go in and come out typed by the declaration. For what a [var](vars.md) would grow into (map nodes, prices seen, a log), looked up by key or by clause. Unprotected.
...
The Lua types live in your declaration alone. The file holds an `INTEGER` where you said `boolean` and says nothing about which column is which. The declaration reads them back, re-read every load because your code declares it every load. A `:put` is typed on the way in and on the way out. A string in an `integer` column is refused naming the column and its type. A column no declaration names any more comes back as the file holds it.
...
| Rule | Detail |
|---|---|
| Interned per addon by name | `"Nodes"` and `"nodes"` are one table in the file and one Table here. Two `:create()`s of one name are `==`. The later declaration is the one both read with. |
...
| The one snapshot | `nodes:info()` is `{name, columns, key, indexes}`. `columns` is an array of `{name, type}` in declaration order, with the declared word (`boolean`, not `INTEGER`). `key` and each entry of `indexes` are arrays of column names. A copy built every call. After a second `:create()` it reads the later declaration. |
...
| `nodes:list(clause, ...)` | `table[]` | Unprotected | Every row the clause keeps. Empty rather than `nil`. Reads at most the [row cap](README.md#the-caps), raising naming `LIMIT` past it. |
...
- [The file](README.md) — the caps, the sandbox, and when a row is on disk.
