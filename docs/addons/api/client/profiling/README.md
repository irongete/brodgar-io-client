...
are *frame sampling*: they exist only while [the switch](../README.md#client) is on. The five
...
per-scope and per-widget figure the same way, as does arming the switch. The five counters are unaffected —
