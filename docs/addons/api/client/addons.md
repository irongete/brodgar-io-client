...
| A bundle | A manifest with `"bundle": true` requires none of its `dependencies`: they order its load like any, and one that is off, missing or below its minimum is, to it, what an absent optional dependency is. Its `:api()` is `nil`, and one stopped mid-session leaves the bundle running ([bundles](../../guides/bundles.md)). |
