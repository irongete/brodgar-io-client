...
`hafen.virtual():widget():add(widget, anchor)` takes a [Widget](../ui/widget.md) off the flat UI (a window of your own, a control you built, one of the client's own windows) and stands it in the 3D world on the client-only core a [sprite](sprites.md) uses: the one kind in the section that is not a picture.
...
| Rule | Detail |
|---|---|
| Two objects, and a refusal names which | `panel has no verb '…'` lists what a thing in the world answers; `widget has no verb '…'` what one on the screen does. `panel:position()` is a [Position](../position.md); [`widget:position()`](../ui/widget.md#read-methods) is pixels within its parent. |
| World size | From the widget's [design pixels](../ui/pixels.md) at a hundred pixels to the tile, so a default `hafen.ui():window()` stands about two tiles across whatever interface scale the user runs. |
...
| Popups open inside the panel | A dropdown's list, a right-click menu and a tooltip resolve against the nearest root, the surface, so a list longer than the panel is clipped at its edge. Size the panel for what it opens. |
...
| The toggle stays the client's | Tab and the menu button open and close a standing window, and the menu tick tells the truth. [Replace](../ui/replace.md) takes the toggle; standing decides where the thing is drawn, so the two compose: stand the view you replaced with, and the client's key drives a panel in the world. |
...
| Outside the view, not drawn | No `Draw` handler, no offscreen pass. `Update` keeps firing, since ticking is logic in the widget tree, so nothing drifts out of date and the picture is correct the instant it returns. |
