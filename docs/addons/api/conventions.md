...
| The root screen | `pagina:parent(nil)`. |
...
The type is checked, never what Lua would convert to: `session:kin():add(1234)` is refused (a hearth secret is a string), `entry:value("42")` is taken, a radio row may be labelled `"061.8"`. The refusal names the verb, the parameter and the conversion you meant (`tostring(n)`, `tonumber(s)`).
...
The client holds more than one account and draws one, so what belongs to one character is reached through the [Session](session.md) that names it ([`session:world()`](world.md), [`session:player()`](player.md), [`session:kin()`](kin.md)), and a read says which character it is about. What belongs to the screen stays where it was: one pointer, one scene. Your addon is the client's, loaded once beside every session, and nothing of yours is torn down when the screen moves.
...
| Handles | Live, bridge-owned proxies with methods (`hafen.ui():window()`, `hafen.timer():every`, `hafen.event():on`), released on disable or reload; so is every object a read hands you, re-resolving rather than holding a value. Userdata with a closed vocabulary: an unknown name raises naming what it answers, nothing can be written onto it (nothing deletes a handle's own `:cancel()`), and `tostring(handle)` names the thing (`Timer(every 5s)`, `Options(video)`, `Sub(GobAdded)`). |
| A table the bridge owns and you write into | [`hafen.store():var(name)`](store/vars.md#read-and-write): neither copy nor proxy but the table that goes to disk, so assigning into it is saving, and the one place a typo on a key is silent and persisted. |
...
A read returns `nil`, or an empty table for a list verb, when the data is not available yet: before the world loads, before a HUD widget streams in, while a resource resolves. Reads never throw a loading error. Much character-sheet data (meters, food, skills, quests, wounds) streams in a beat after `SessionEnteredWorld`: read it on a timer or subscribe to its [event](event/bus/README.md).

## Threading

Your Lua runs either on the step (inside no character's UI, able to reach every one) or answering something (a draw, a press, a drop, a console line) inside the one character's UI that dispatched it, the only one it may reach. Taking a second is refused at the line that tries; nothing blocks. [Threading](threading.md) says which handler is which.

## The permission model

| Rule | Detail |
|---|---|
| Protected | A verb that starts an action the player could have performed runs only if your addon declared its permission key in the [manifest](../manifest.md#the-manifest) and the user enabled it. Such an addon is disabled the first time the client sees it and enabling it raises a consent dialog; one that never declared the key gets an error naming the verb and the key. |
...
| Outside the client, the same words | `http.get`, `http.post`, `websocket.connect`, `voice.connect`, with the `network` host allowlist as the key's argument: the key says whether, the hosts where, read as one consent line, and the approved list gates the call. |
