...
| Rule | Detail |
|---|---|
| `message` | A string, or any value converted as `tostring` does. A handle prints as itself (`Gob(1234)`, `Timer(every 5s)`). A plain table prints as a Lua table description, so encode it with [`hafen.json`](json.md) to see inside. A missing `message`, or an explicit `nil`, raises. |
...
| Rate-limited in-game | Eight lines a second per addon. Past that the lines are counted. One line at the end of the second says how many were dropped and that the terminal has them. The terminal half is never limited. |
...
- [`hafen.json`](json.md) — turning a table into a printable string.
