...
writes on [Look](look.md) — `gob:scale(k)`, `gob:visible(b)` and `gob:tint(c)`; what it is made of is
[`gob:materials()`](materials.md), the variable-material slots the server dressed it in, each readable and
dressable in another resource by name; and what stands at it is the collection
[`gob:overlay()`](overlay.md). All of them are unprotected, all of them are written on the object rather
than a character, and none of them changes anything on this page: a resized, hidden, tinted or re-dressed
object still answers every read above.
...
- [Materials](materials.md) — the material slots it is drawn in: the server's, and one of your own by name
