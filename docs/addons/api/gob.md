...
A Gob is one thing in the world (a tree, a boulder, an animal, a player's body), addressed through the world of the character that sees it: [`session:world():gob()`](world.md#objects) is the collection, `:get(id)` names one by id, the rest of the collection finds the ones you have no id for.
...
Each character holds its own copy of the object against its own map, so a read is computed by one of them: the character whose door handed you the Gob.
...
| Expression | Returns |
|---|---|
| `session:world():gob():get(id)` | The Gob for that id. Never `nil`. |
| `session:player():gob()` | That character's own Gob; `nil` before its session is in the world ([`session:player`](player.md)). |
| `session:world():gob():list(filter)`, `:within(radius, filter)` | An array of Gobs. |
| `session:world():gob():nearest(filter)` | The nearest Gob to that character, or `nil`. |
| A `GobAdded` or `GobRemoved` handler | The Gob that spawned or despawned ([events](event/bus/world.md#world)). |
...
`gob:sdt()` answers the raw state bytes the server sent with the gob's resource (a crop's stage, a gate's leaf, a stockpile's count) as a 1-based array of `0..255` numbers. What the bytes mean belongs to the resource's own published code; the client never decodes them.
...
> **Three facts share this answer, and the verb does not say which you got.** Usually the rings are the resource's collision shapes. A resource may carry no collision shape and still carry a click-box: a felled log (`gfx/terobjs/log`) blocks nothing and answers an 18×4 rectangle. An object may carry no shape of its own and be sent one per object by the server ([a building site](#a-shape-the-server-sent)). What the three share is the promise: world units, turned by the facing, placed where it stands. Never read a shape as proof that the object blocks movement.
...
A building site (`gfx/terobjs/consobj`, the same resource for every building) has no footprint of its own: what goes up on the spot is sent with the object in its [state bytes](#state), and the stakes you see are the resource's own code drawing that shape. `gob:hitbox()` reads it, so a site answers the outline of the building it will become.
...
assert(main_session:player():gob() == main_session:world():gob():get(my_id))
