...
Read and toggle the minimap icon registry the client's Icon settings window edits. A category is one kind of gob icon (a boar, a fir tree, a player) with two flags: `show`, draw it on the minimap, and `notify`, play a sound and a chat message when one appears.
...
| A category is a resource | Where an icon resource publishes several variants, they are one category and a write reaches all: the engine keys them by resource plus an opaque sub-id no name could address. |
