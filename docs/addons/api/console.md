...
Both directions onto the client's command line (`:` opens it, and `:lua`, `:reload`, `:fs` and yours are console commands). `hafen.console()` registers a command, unprotected. [`session:console():run(line)`](#run-a-line-protected) says a line at one character's console, protected.

```lua
local greet_command = hafen.console():on("greet", function(arguments)
  hafen.log():write("hello, " .. (arguments[1] or "world"))
end)
...
|---|---|---|---|
| `hafen.console():on(name, fn)` | [`Sub`](event/README.md#subscribe) | Unprotected | Route the console command `:name words…` to `fn(arguments)`. |

| Rule | Detail |
|---|---|
| `name` | A single word: no spaces, not empty. |
| `arguments` | A 1-based table of the whitespace-split words after the command name. `"quoted words"` group into one, `\` escapes the next character. The name itself is not in the table. |
...
| Two addons, one name | The newest subscription wins and takes it over, and the console records the reassignment. The replaced subscription ends: it leaves that addon's `:list()`, `command:off()` on it does nothing, nothing of it reads alive. |
...
| Rule | Detail |
|---|---|
| `line` | The command and the rest of the line without the opening colon: `run("reload")`, not `run(":reload")`. The client's own splitter splits it, so `arguments` arrive as for a typed command. `:lua` reads the literal text back. |
...
| A command that fails is not your error | `run("zzz")` returns normally and writes `zzz: no such command` to that character's System log and on-screen notice. Nothing is raised. Before the HUD is up the log half is dropped, the notice half shows. Once the HUD is up the client logs notices to System as well. One failed command leaves two identical System lines. Read the newest, do not count lines. |
...
| Why protected | `console.run` is the widest key in the [catalogue](../guides/permissions.md#the-catalogue). It covers every command the client dispatches, on any character. `:lua` is one, and it evaluates against the whole standard library outside your sandbox. The line the user reads when they enable you says so. |
