...
One character's maneuver-deck builder (its Martial Arts and Combat Schools tab) and who it is fighting, reached through its [session](session.md): what the character knows, what its loaded school has dealt to each hotkey, what it costs, and the opponent.
...
|---|---|---|---|
| `session:fight():maneuver():list(filter)` | `Maneuver[]` | Unprotected | Every maneuver and attack that character knows. |
...
| Rule | Detail |
|---|---|
| `filter` | A string [filter](conventions.md#the-filter-argument) matches the resource name and the display name; on the deck it matches the maneuver in the slot, so one needle finds the same maneuver through either door. |
| No `:get` | A maneuver is addressed by nothing you hold: a string is a search, a position is `:list()[n]`. `#deck()`, `deck()[n]` and `ipairs(deck())` are [refused](conventions.md#collections-the-noun-is-the-kind-the-verb-is-how-many); `deck():list()` is the array. |
...
A card is a place in the layout, not the maneuver in it: it keeps answering `:wire()` and `:key()` when the hotkey is emptied, while the maneuver half goes `nil` and `:exists()` goes `false`.
...
| `card:maneuver()` | `Maneuver \| nil` | Unprotected | The maneuver dealt here. |
| `card:res()` | `string \| nil` | Unprotected | That maneuver's resource name. |
| `card:name()` | `string \| nil` | Unprotected | That maneuver's display name. |
...
| Method | Returns | Permission | Description |
|---|---|---|---|
| `summary:used()` | `number \| nil` | Unprotected | Action points the loaded school spends: the total the window paints beside the cap, the sum of `maneuver:used()` over every known maneuver. |
...
[`session:study():summary()`](study.md#the-summary) is a summary of the same kind: a live object with its own `:exists()` and `:info()`, interned on its window, `nil` while the window is not up.
