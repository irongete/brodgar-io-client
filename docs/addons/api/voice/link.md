...
A voice link is one connection to one voice server. It is built bare by `hafen.voice():connection(url)`, configured by chained setters, opened by `:connect()`, heard through `:on`, and ended by `:close()` or by the server. What it sends and plays is [the mic and the mix](audio.md). Who it relates you to is [peers](peers.md).
...
| Asynchronous | `:connect()` returns at once with the link `"connecting"`. Every edge arrives through `:on` a frame or more later on the [step](../threading.md), holding no tree. A link needs no login to exist: one opened from `Load` is live on the login screen and outlives every character. It has nothing to report until its session is in the world. |
...
| `Error` | The link failed, or never opened. State `"closed"`. | `event`: `event:error()` is one line saying why. A host that does not resolve, or an address refused. A microphone that will not open. A server that refused the hello or speaks another protocol version. A handshake past the timeout. |
...
| State | Effect |
|---|---|
| Open | Reads `"closing"` while the close is sent to the server and the audio stops. `Close` fires with `""` once done. Inside the handler the microphone is released and the server slot is free. A `:connect()` to the same server is not refused. |
