...
| Before the hotbar exists | Before the character is in the world, and briefly after a reload, every slot reads as empty. At login the occupied slots stream in a beat later as a burst of `ActionbarChanged`. |

## The page

The client draws twelve buttons at a time and pages through the 144: page `p` is slots `(p-1)*12+1 .. p*12`, turned by the client's `Go to page N` keys or `:page(n)`, both 1-based.
...
| Asynchronous | `res` and `clear` send to the server, which echoes before the slot changes: the next line reads the old content, and `:res(name):use()` in one chain activates what was there before. React to `ActionbarChanged` on that slot, or wait a beat. |
...
A [menu entry an addon added](menugrid.md#write-unprotected) sits on the bar too, drawing its icon in the slot and running its Lua when the slot's key is pressed. The server owns the bar and has never heard of the entry, so the client holds the slot: it draws over the server's content and hands it back untouched when the hold ends.

```lua
local session = hafen.session():current()
local dig_entry = session:menugrid():add("dig"):name("Auto-dig"):icon(hafen.asset():get("dig.png"))
dig_entry:on("use", function() hafen.log():write("dug") end)
...
| While held | `slot:res()` is the entry's `addon/…` identity, `slot:name()` the name you gave it, `slot:empty()` is false, and pressing the slot (key, click, `slot:use()`) runs your [`pagina:on("use", fn)`](menugrid.md#a-click-runs-your-lua) handlers: one button in two places through the same code. |
...
Every row but the last puts back what the server has in the slot, unchanged. In the last row the message being handled is the server assigning or clearing that slot, so what it wrote stands and the hold ends; `slot:res(name)` on a slot you hold is that row, a beat later.
...
| One entry per slot | The live hold chains, the record does not: two addons holding one slot in turn are one remembered entry, the last written, and the bar comes back with that one. |
...
| Disabling keeps the slots | They go back to the server's content as the addon is torn down and the rows wait, dormant, with its options and hotkeys; enabling puts every button back as your `:add` runs. A right-click on a dormant slot forgets it. |
