...
| Setter | Meaning |
|---|---|
| `ov:draw(fn)` | `fn(g, gob, sx, sy)` runs every frame at that point, painting with [`g`](ui/drawing.md); `sx, sy` is in [design pixels](ui/pixels.md), like everything `g` takes — the projected point to the whole device pixel, its fraction [carried under the whole painter](ui/drawing.md#draw-methods), so what you draw glides with the object |
