...
| Malformed input raises | `JSON: <message> at offset <n>`. Malformed means JSON's own grammar. `01`, `+5`, `.5`, `5.` and `1e` are refused. So is a `\u` escape that is not exactly four hex digits (`\uZZZZ`, `\u-123`). Wrap untrusted input in `pcall`. |
...
```lua
local untrusted = '{"name": "Bob", "level": }'          -- text from the network, a file, another addon
...
| Rule | Detail |
|---|---|
| Strict | Raises rather than emitting a placeholder for a value JSON cannot carry. A function, userdata or thread. A reference cycle, or a table past the depth cap. A non-finite number. A key of a kind JSON cannot name, or two keys spelling one name. A document past the size cap. Wrap it in `pcall` if the value might hold one. |
...
```lua
local value = { name = "Bob", tags = { "kin", "online" } }
...
> **What [`hafen.store`](store/vars.md) writes is not strict.** The timer's write puts the literal string `"<cycle>"` or `"<too deep>"` where `encode` would raise. It logs the path it degraded. A saved table can come back as text. `hafen.store():flush()` refuses instead, naming the path.
