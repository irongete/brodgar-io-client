...
```lua
local sheet = hafen.ui():sheet()
sheet:rule("chat"):color{190, 210, 190}     -- ...or, once the sheet is installed, changes it live
sheet:rule("chat"):release()                -- and this gives that one rule back
```
...
- **Ask the addon what it calls things**, or point the **`widgetstack`** addon at it
  ([the maintainer's addons](../examples.md)): names are part of what an addon publishes, like its console
