...
`:lua <expression>` evaluates against the live API and prints the result as JSON. It runs as your own console rather than addon code: nothing sandboxed, every protected verb works.
...
[`hafen.event()`](../api/event/README.md#read-what-you-are-listening-to) covers the bus and the message streams, [`hafen.console()`](../api/console.md#read-what-you-registered) your commands, [`hafen.timer()`](../api/timer.md) your timers. A count that climbs while the client runs is a subscription made outside your file body or `Load`. Usually it is a handler that subscribes on each fire.

## Name a widget you are pointing at

Enable the `widgetstack` addon and hover. It reports the widget's [role](../api/ui/selectors.md#roles), class, caption or displayed words, `[res=]` and the captioned window it sits in. Then it offers every selector it can build from those, most specific first, each resolved before shown and ready to paste into `:lua`. It offers chains first (the captioned window, then the widget), since a flat step usually matches several. Its `freeze` hotkey holds the stack still while you move the mouse. Two answers from [the inspector's page](../api/ui/selectors.md): hovering a window's frame gives the frame, not the window, since the chrome is a widget of its own. And most widgets have no role.
...
Arm the client's profiler: Options ▸ Game ▸ Client, or [`hafen.client():options():client():profiling(true)`](../api/client/README.md#client). Then read [`hafen.client():profiling()`](../api/client/profiling/README.md). It has the frame, the render passes, per-widget cost, and what each addon's Lua cost, most expensive first.
