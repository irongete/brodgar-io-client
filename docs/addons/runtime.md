...
| The string metatable is the client's | `getmetatable("")` answers a sentinel rather than the table, so the vocabulary `("x"):upper()` resolves through is out of reach and no addon can move another's strings. Your own `string` table is yours: writing to it moves `string.upper(text)` in your addon alone, and leaves `("x"):upper()` on the client's. |
